...
def create_archive_memory(
    original_path: str, archive_path: str, category: str, reason: str
):
...
def task1_archive_memories():
    ...
    for file_entry in temp_manifest.get("files", []):
        ...
        create_archive_memory(
            original_path=original, archive_path=archive, category="temp", reason=reason
        )
        temp_count += 1
...
def task2_doc_move_memories():
    ...
    doc_moves = [
        ...
        (
            "lab2_analysis_gemini.md",
            "docs/analysis/lab2-analysis-gemini.md",
            "analysis",
        ),
        (
            "EMA_FEATURE_MIGRATION_PLAN.md",
            "docs/features/emas/ema-feature-migration-plan.md",
            "features",
        ),
        (
            "EMA_MIGRATION_SESSION_SUMMARY.md",
            "docs/features/emas/ema-migration-session-summary.md",
            "features",
        ),
    ...
    conversion_archives = [
        (
            ...
            ".archive/documentation/2026-02-03/conversion/conversion_checkpoint.json",
            "conversion-artifacts",
        ),
        (
            "docs/conversion_errors.json",
            ".archive/documentation/2026-02-03/conversion/conversion_errors.json",
            "conversion-artifacts",
        ),
        (
            "docs/conversion_notes.md",
            ".archive/documentation/2026-02-03/conversion/conversion_notes.md",
            "conversion-artifacts",
        ),
    ]
...
def task3_phase_snapshot():
    ...
    scripts_manifest = json.load(open(".archive/scripts/manifest.json"))
    duplicates_manifest = json.load(open(".archive/duplicates/manifest.json"))
    dup_report = json.load(
        open(".planning/phases/16-repository-cleanup/duplicates_report.json")
    )
    sim_report = json.load(
        open(".planning/phases/16-repository-cleanup/similarity_report.json")
    )
...
"""
    ...
    print("Phase 16 snapshot created:")
...
def verify_memories():
    ...
    results = client.search(
        query="Phase 16 temp files archived", user_id="ta_lab2_system", limit=5
    )
    ...
    results = client.search(
        query="API_MAP.md moved to", user_id="ta_lab2_system", limit=5
    )
    if isinstance(results, dict) and "results" in results:
        results = results["results"]
    print(f"  Found {len(results)} results")
    ...
    results = client.search(
        query="Phase 16 repository cleanup completed", user_id="ta_lab2_system", limit=5
    )
    if isinstance(results, dict) and "results" in results:
        results = results["results"]
    print(f"  Found {len(results)} results")
