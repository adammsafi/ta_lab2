...
class TA_Lab2_Memory:
    ...
    def get_recent_decisions(
        ...
        categories = (
            [category] if category else ["architecture", "features", "regimes", "data"]
        )
...
if __name__ == "__main__":
    ...
    add_dec.add_argument(
        "category", choices=["architecture", "features", "regimes", "data"]
    )
    ...
    if args.command == "add-decision":
        dec_id = memory.add_decision(
            args.category, args.description, args.rationale, args.platform
        )
        ...
        for goal in summary["active_goals"][:3]:
            print(
                f"  • [{goal.get('priority', 'medium').upper()}] {goal['description']}"
            )
