...
def get_releases(api_key: str, limit=10000):
    r = requests.get(f"{BASE}/releases", params={"api_key": api_key, "file_type": "json", "limit": limit}, timeout=30)
    r.raise_for_status()
    data = r.json()
    if "releases" not in data: raise RuntimeError(f"no 'releases' in payload: {data}")
