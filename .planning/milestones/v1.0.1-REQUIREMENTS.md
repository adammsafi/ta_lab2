# Requirements Archive: v1.0.1 Macro Regime Infrastructure
...
- [x] **MINT-03**: Macro regime policy entries added to DEFAULT_POLICY_TABLE with substring matching for partial patterns
...
- [x] **XAGG-01**: BTC/ETH 30d rolling correlation exposed as a queryable feature
- [x] **XAGG-02**: Cross-asset correlation matrix: 30d rolling average pairwise correlation with high-correlation flag (>0.7)
- [x] **XAGG-03**: Aggregate funding rate signal: average funding rate across tracked BTC/ETH perp pairs with z-score
- [x] **XAGG-04**: Crypto-macro correlation regime: rolling 60d BTC returns vs VIX, DXY, HY OAS with sign-flip anomaly detection

### Event Risk Gates (GATE)

- [x] **GATE-01**: `dim_macro_events` table seeded with FOMC, CPI, NFP dates for 2026-2027
- [x] **GATE-02**: FOMC event gate: size_mult reduction applied +/-24h around FOMC meetings
- [x] **GATE-03**: VIX spike gate: VIX > 30 triggers REDUCE, VIX > 40 triggers FLATTEN
- [x] **GATE-04**: Carry unwind velocity gate: DEXJPUS daily z-score > 2.0 triggers REDUCE; > 3.0 triggers FLATTEN
- [x] **GATE-05**: Data freshness gate: WARN at 48h, disable macro regime at 96h
- [x] **GATE-06**: Credit stress gate: HY OAS 5d z-score > 1.5 applies 0.7; > 2.5 applies 0.4
- [x] **GATE-07**: CPI release day gate: size_mult reduction +/-24h around CPI releases
- [x] **GATE-08**: NFP release day gate: size_mult 0.75 on first Friday of each month
- [x] **GATE-09**: Composite macro stress score (0-100) with tiered response

### Observability & Monitoring (OBSV)

- [x] **OBSV-01**: Macro regime display in Streamlit dashboard
- [x] **OBSV-02**: Telegram alert on macro regime transition
- [x] **OBSV-03**: FRED data freshness display in pipeline monitor
- [x] **OBSV-04**: Macro regime as drift attribution source in DriftMonitor
- [x] **OBSV-05**: Macro regime timeline chart with colored bands
- [x] **OBSV-06**: FRED data quality dashboard tab
...
**Shipped:** 55 of 55 v1.0.1 requirements
**Adjusted:** None -- all requirements implemented as specified
**Dropped:** None

---
*Archived: 2026-03-03 as part of v1.0.1 milestone completion*
