...
```
  ...
  <verify>wc -l docs/REORGANIZATION.md | grep -E "^[0-9]{3,}" && grep -q "Migration Guide" docs/REORGANIZATION.md && grep -q "Verification" docs/REORGANIZATION.md && python -c "import re; content=open('docs/REORGANIZATION.md').read(); tables=re.findall(r'\|[^\n]+\|[^\n]+\|[^\n]+\|', content); file_rows=[r for r in tables if 'ProjectTT' in r or 'Data_Tools' in r or 'fredtools' in r or 'fedtools' in r or '.py' in r or '.docx' in r]; print(f'Found {len(file_rows)} file documentation rows'); assert len(file_rows) >= 50, f'Expected 50+ file rows documenting 128 files, found {len(file_rows)}'"</verify>
  <done>REORGANIZATION.md complete with all 4 source directories (128 files documented), migration guide with import mappings, archived file alternatives, and verification section (500+ lines)</done>
</task>
...
3. File listings match manifest totals (62 + 51 + 6 + 9 = 128 files documented)
...
- Every file from external directories has documented destination (128 total)
