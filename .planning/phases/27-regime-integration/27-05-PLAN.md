...
must_haves:
  ...
  artifacts:
    - path: "src/ta_lab2/scripts/regimes/refresh_cmc_regimes.py"
      provides: "Complete regime refresh script with hysteresis, flips, and stats"
      min_lines: 300
  key_links:
...
<objective>
Wire hysteresis, flip detection, and stats into the core refresh script to produce the complete regime pipeline.

Purpose: Plans 03 and 04 built the core computation and the analytics modules separately. This plan integrates them: apply hysteresis to smooth labels, detect+store flips, compute+store stats, and add all remaining CLI flags.
Output: Complete refresh_cmc_regimes.py that writes to all 3 regime tables.
</objective>
...
<task type="auto">
  <name>Task 2: Wire flips and stats into the main refresh loop</name>
  <files>
    src/ta_lab2/scripts/regimes/refresh_cmc_regimes.py
  </files>
  <action>
...
3. Print final summary after all assets processed:
   ...
   - Total regime rows, flips, stats rows written
   - Time elapsed
...
with engine.connect() as conn:
    for t in ['cmc_regimes', 'cmc_regime_flips', 'cmc_regime_stats']:
...
```
  ...
  <done>
- Complete refresh pipeline writes to all 3 regime tables
...
<success_criteria>
- All 3 regime tables populated for at least one asset
