...
Purpose: Establish the database schema foundation that all subsequent regime plans depend on. Without these tables, nothing can be written.
Output: 5 SQL files in sql/regimes/ directory, tables created in PostgreSQL.
</objective>
...
<task type="auto">
  ...
  <action>
Create `sql/regimes/` directory and write 3 DDL files:
...
with engine.begin() as conn:
    for f in ['sql/regimes/080_cmc_regimes.sql', 'sql/regimes/081_cmc_regime_flips.sql', 'sql/regimes/082_cmc_regime_stats.sql']:
...
```
  ...
  <done>Three regime tables exist in PostgreSQL with correct PKs and columns.</done>
</task>
...
<verification>
- All 5 SQL files exist and are syntactically valid
- All 3 new tables created with correct PKs
- Signal tables extended with regime_key
- dim_signals extended with regime_enabled
- All SQL is idempotent (IF NOT EXISTS / IF NOT EXISTS)
</verification>

<success_criteria>
- 3 regime tables exist in PostgreSQL: cmc_regimes, cmc_regime_flips, cmc_regime_stats
