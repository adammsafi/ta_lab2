...
- CRITICAL: After pivot, period column values may be int or text. Cast to int before renaming:
  ```python
  pivot.columns = ["id", "ts"] + [f"{price_col}_ema_{int(p)}" for p in sorted(int(c) for c in pivot.columns[2:])]
  ```
  This ensures correct column ordering even if `period` is TEXT in the DB (avoids alphabetic sort where "200" < "50").
...
# Verify numeric ordering: 20 < 50 < 200 (not alphabetic where '200' < '50')
ema_cols = [c for c in result.columns if 'ema' in c]
assert ema_cols == ['close_ema_20', 'close_ema_50', 'close_ema_200'], f'Wrong column order: {ema_cols}'
print('PASS: pivot_emas_to_wide works correctly with numeric ordering')
print(result)
"
```
...
```
  ...
  <done>
- pivot_emas_to_wide correctly transforms long-format (id, ts, period, ema) to wide-format (id, ts, close_ema_X) with numeric column ordering
...
- Pivot uses int-based sorting (not alphabetic) for period column names
