...
- For each EMA pair from the `agree` DataFrame:
  - Also calls `lead_lag_max_corr(daily_df, ema_a, ema_b)` for the lead-lag stat
...
- compute_and_write_comovement computes pairwise EMA comovement and writes to cmc_regime_comovement
