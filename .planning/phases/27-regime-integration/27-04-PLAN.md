...
must_haves:
  truths:
    ...
    - "Comovement stats are computed via compute_ema_comovement_stats() and written to cmc_regime_comovement"
  artifacts:
    ...
    - path: "src/ta_lab2/scripts/regimes/regime_comovement.py"
      provides: "DB-backed comovement stats computation and writing"
      exports: ["compute_and_write_comovement", "write_comovement_to_db"]
  key_links:
    - from: "src/ta_lab2/scripts/regimes/regime_comovement.py"
      to: "src/ta_lab2/regimes/comovement.py"
      via: "import compute_ema_comovement_stats, lead_lag_max_corr"
      pattern: "from ta_lab2.regimes.comovement import"
    ...
    - from: "src/ta_lab2/scripts/regimes/regime_comovement.py"
      to: "cmc_regime_comovement table"
      via: "scoped DELETE + INSERT"
      pattern: "DELETE FROM.*cmc_regime_comovement"
---

<objective>
Build the hysteresis tracker, flip detection, regime stats, and comovement stats modules.

Purpose: The current hysteresis in resolver.py is a stub that always accepts changes. Real hysteresis prevents rapid regime flipping (important for avoiding whipsaw in position sizing). Flips and stats tables provide analytics for regime behavior analysis. Comovement stats (EMA alignment, sign agreement, lead-lag) are mandated by CONTEXT.md.
Output: HysteresisTracker class, regime_flips.py, regime_stats.py, regime_comovement.py modules.
</objective>
...
Also add a convenience function to determine if switching from old_key to new_key would tighten risk.
Use the public `resolve_policy_from_table(key, policy_table)` API from resolver.py (do NOT import the
private `_match_policy` function). Compare the resolved size_mult values:

```python
from ta_lab2.regimes.resolver import resolve_policy_from_table
...
def is_tightening_change(old_key: str, new_key: str, policy_table) -> bool:
    ...
    old_policy = resolve_policy_from_table(old_key, policy_table)
    new_policy = resolve_policy_from_table(new_key, policy_table)
...
<task type="auto">
  <name>Task 2: Create regime_flips.py, regime_stats.py, and regime_comovement.py</name>
  <files>
    src/ta_lab2/scripts/regimes/regime_flips.py
    src/ta_lab2/scripts/regimes/regime_stats.py
    src/ta_lab2/scripts/regimes/regime_comovement.py
...
**regime_comovement.py** - Compute and store EMA comovement stats (CONTEXT.md mandate):

`compute_and_write_comovement(engine, asset_id, daily_df, tf='1D')`:
- Takes the daily DataFrame with EMA columns (close_ema_20, close_ema_50, close_ema_100)
- Calls `compute_ema_comovement_stats(daily_df)` from `ta_lab2.regimes.comovement`
- `compute_ema_comovement_stats` returns a dict with keys including 'corr' (a square DataFrame of
  pairwise Spearman correlations indexed by EMA column name) and 'agree' (sign agreement rates)
- For each EMA pair (ema_a, ema_b) from the `agree` DataFrame:
  - Extract correlation = stats['corr'].loc[ema_a, ema_b] to populate the correlation column
  - Extract sign_agree_rate = stats['agree'].loc[ema_a, ema_b]
  - Call `lead_lag_max_corr(daily_df, ema_a, ema_b)` which returns (best_lead_lag, best_lead_lag_corr)
  - Extract n_obs from the length of the non-null intersection of ema_a and ema_b columns
- Builds output DataFrame with columns matching cmc_regime_comovement schema:
  - id, tf, ema_a, ema_b, correlation, sign_agree_rate, best_lead_lag, best_lead_lag_corr, n_obs, computed_at
- Returns the output DataFrame

`write_comovement_to_db(engine, como_df, ids, tf='1D')`:
- Scoped DELETE + INSERT on cmc_regime_comovement:
  ```python
  with engine.begin() as conn:
      conn.execute(text("DELETE FROM public.cmc_regime_comovement WHERE id = ANY(:ids) AND tf = :tf"),
                   {"ids": ids, "tf": tf})
  como_df.to_sql("cmc_regime_comovement", engine, schema="public", if_exists="append", index=False, method="multi")
  ```
- Return rows written

All modules should use logging and handle empty DataFrames gracefully.
  </action>
  <verify>
```python
python -c "
...
- compute_and_write_comovement extracts correlation via stats['corr'].loc[ema_a, ema_b] and sign_agree_rate via stats['agree'].loc[ema_a, ema_b], calls lead_lag_max_corr for each pair, and writes to cmc_regime_comovement
- All writers use scoped DELETE + INSERT pattern
  </done>
</task>
...
- Comovement module calls compute_ema_comovement_stats() and lead_lag_max_corr()
...
- Comovement stats written to cmc_regime_comovement for EMA pairs
</success_criteria>
