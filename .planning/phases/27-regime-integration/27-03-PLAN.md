...
must_haves:
  truths:
    ...
    - "Young assets with disabled layers get proxy-inferred labels via infer_cycle_proxy/infer_weekly_macro_proxy instead of None"
  ...
  key_links:
    ...
    - from: "src/ta_lab2/scripts/regimes/refresh_cmc_regimes.py"
      to: "src/ta_lab2/regimes/proxies.py"
      via: "import infer_cycle_proxy, infer_weekly_macro_proxy"
      pattern: "from ta_lab2.regimes.proxies import"
    - from: "src/ta_lab2/scripts/regimes/refresh_cmc_regimes.py"
...
<objective>
Build the core refresh_cmc_regimes.py script that orchestrates regime labeling from DB data, including proxy fallback for young assets.

Purpose: This is the central script that connects the existing regime module to the DB-backed pipeline. It loads bars+EMAs, runs L0-L2 labeling, resolves policy via tighten-only semantics, and writes results to cmc_regimes. Young assets without enough bar history use proxy inference (infer_cycle_proxy, infer_weekly_macro_proxy) rather than leaving labels as None.

NOTE: Incremental refresh with watermarks is deferred to a future phase per RESEARCH.md recommendation. For v0.7.0, this script performs full-recompute per asset on each run. Regime computation is fast enough (< 1 second per asset) that watermarks are not a bottleneck for daily refresh.
Output: Working refresh_cmc_regimes.py with CLI, per-asset processing, proxy fallback, and DB write.
</objective>
...
<task type="auto">
  <name>Task 1: Build compute_regimes_for_id function with proxy fallback</name>
  <files>
    src/ta_lab2/scripts/regimes/refresh_cmc_regimes.py
  </files>
  <action>
...
5. **PROXY FALLBACK for young assets (CONTEXT.md mandate: "Use proxies for young assets without enough bar history"):**
   When a layer is disabled by data_budget, attempt proxy inference instead of leaving the label as None:
   ```python
   from ta_lab2.regimes.proxies import (
       ProxyInputs, infer_cycle_proxy, infer_weekly_macro_proxy
   )

   # L0 proxy: If L0 disabled, use broad market proxy to cap exposure
   if not ctx.enabled_layers["L0"]:
       # Load BTC (id=1) weekly data as market proxy (crypto broad market)
       try:
           market_weekly = load_and_pivot_emas(engine, 1, "1W", [20, 50, 200], cal_scheme=cal_scheme)
           market_weekly_bars = load_bars_for_tf(engine, 1, "1W", cal_scheme=cal_scheme)
           market_weekly = market_weekly_bars.merge(market_weekly, on=["id", "ts"], how="left")
           proxy_inp = ProxyInputs(child_daily=daily, market_weekly=market_weekly)
           proxy_out = infer_cycle_proxy(proxy_inp)
           # proxy_out.l0_cap reduces gross_cap (applied during policy resolution below)
           logger.info(f"  L0 proxy: l0_cap={proxy_out.l0_cap}")
       except Exception as e:
           logger.warning(f"  L0 proxy failed: {e}")
           proxy_out = None

   # L1 proxy: If L1 disabled, use parent (BTC) weekly regime to tighten size
   if not ctx.enabled_layers["L1"]:
       try:
           parent_weekly = load_and_pivot_emas(engine, 1, "1W", [20, 50, 200], cal_scheme=cal_scheme)
           parent_weekly_bars = load_bars_for_tf(engine, 1, "1W", cal_scheme=cal_scheme)
           parent_weekly = parent_weekly_bars.merge(parent_weekly, on=["id", "ts"], how="left")
           proxy_inp_l1 = ProxyInputs(child_daily=daily, parent_weekly=parent_weekly)
           proxy_out_l1 = infer_weekly_macro_proxy(proxy_inp_l1)
           logger.info(f"  L1 proxy: l1_size_mult={proxy_out_l1.l1_size_mult}")
       except Exception as e:
           logger.warning(f"  L1 proxy failed: {e}")
           proxy_out_l1 = None
   ```
   Apply proxy tightening to policy fields after resolution (proxy_out.l0_cap multiplies gross_cap, proxy_out_l1.l1_size_mult multiplies size_mult).

6. Forward-fill multi-TF labels to daily index:
...
7. For each daily row, resolve policy:
   ...
   - If proxy results exist, apply tightening: `policy.gross_cap *= proxy_out.l0_cap`, `policy.size_mult *= proxy_out_l1.l1_size_mult`
8. Build regime_key: Use the L2 (daily) label as primary key, or compose from available layers
   - If L2 exists, regime_key = L2 label value (e.g., "Up-Normal-Normal")
   - Fallback to L1 if L2 disabled, then L0
9. Compute version_hash: hashlib.sha256 of sorted(policy_table keys) + code version
10. Build output DataFrame with columns matching cmc_regimes schema:
...
11. Return the output DataFrame (or empty DataFrame if no daily data)
...
- Handle case where monthly/weekly DataFrames are empty (young asset with no calendar bars) -- data budget will auto-disable those layers, then proxy kicks in.
...
- Use logging (import logging, logger = logging.getLogger(__name__)) for all info/debug messages.
  ...
  <verify>
Verify the function imports, basic structure, AND regime_key diversity:
```python
python -c "
...
Then run a real integration check for BTC (id=1) to verify labels are NOT all-Sideways:
```python
python -c "
from sqlalchemy import create_engine
import os
from ta_lab2.scripts.regimes.refresh_cmc_regimes import compute_regimes_for_id
from ta_lab2.regimes.resolver import DEFAULT_POLICY_TABLE

engine = create_engine(os.environ['TARGET_DB_URL'])
df = compute_regimes_for_id(engine, 1, DEFAULT_POLICY_TABLE)
if not df.empty:
    unique_keys = df['regime_key'].nunique()
    print(f'regime_key diversity: {unique_keys} unique values out of {len(df)} rows')
    print(f'Distribution: {df[\"regime_key\"].value_counts().head(5).to_dict()}')
    assert unique_keys >= 2, f'SUSPICIOUS: Only {unique_keys} unique regime_key values -- likely NaN column mapping producing all-Sideways labels'
    print('PASS: regime_key has sufficient diversity')
else:
    print('WARNING: Empty DataFrame returned (may indicate missing data for id=1)')
"
```
  </verify>
  <done>compute_regimes_for_id loads data, runs labelers with correct column mapping, applies proxy fallback for young assets, forward-fills multi-TF labels, resolves policy with proxy tightening, and returns DataFrame matching cmc_regimes schema. Regime key diversity confirms labelers are receiving correct column names.</done>
</task>
...
- regime_key has >= 2 unique values for BTC (confirms correct column mapping)
- Proxy fallback logic is reachable (tested with a young asset if available)
</verification>
...
- Young assets with disabled layers get proxy-inferred tightening (not bare None labels)
