...
```
Should show regime refresh would be executed.
  </verify>
  <done>run_daily_refresh.py supports --regimes flag and --all includes regimes after EMAs.</done>
</task>
...
1. DDL for 3 regime tables + signal table extensions (Plan 01)
2. EMA pivot and data loading utilities (Plan 02)
3. Core refresh_cmc_regimes.py with labeling and policy resolution (Plan 03)
4. Hysteresis tracker, flip detection, stats computation (Plan 04)
5. Full refresh pipeline with all 3 tables (Plan 05)
6. Signal generator regime awareness with --no-regime flag (Plan 06)
...
4. Run signal generation with regime context:
```bash
...
6. Check daily refresh orchestrator:
```bash
python -m ta_lab2.scripts.run_daily_refresh --regimes --ids 1 --dry-run -v
```
Verify: Should show regime refresh would be executed.
  </how-to-verify>
...
<success_criteria>
- Full pipeline: refresh_cmc_regimes.py --ids 1 completes and writes to all 3 tables
