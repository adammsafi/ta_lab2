...
must_haves:
  truths:
    ...
    - "If tables are missing, setup script creates and populates them"
  artifacts:
    ...
    - path: "tests/time/test_dim_sessions.py"
      ...
      min_lines: 40
    - path: "src/ta_lab2/scripts/setup/ensure_dim_tables.py"
      provides: "Conditional creation script for dimension tables"
      min_lines: 100
  key_links:
    ...
    - from: "src/ta_lab2/scripts/setup/ensure_dim_tables.py"
      to: "sql/lookups/010_dim_timeframe_create.sql"
      via: "executes SQL seed files"
      pattern: "dim_timeframe"
---

<objective>
Validate dimension tables exist and populate if missing

Purpose: Confirm that dim_timeframe and dim_sessions tables exist in PostgreSQL with expected data. If tables are missing, create and populate them using existing SQL seed files. This is conditional infrastructure setup - create if missing, validate if exists.

Output: Test suite validating dimension table existence + setup script for conditional creation
</objective>
...
# Existing SQL seed files
@sql/lookups/010_dim_timeframe_create.sql
@sql/lookups/011_dim_timeframe_insert_daily.sql
@sql/lookups/012_dim_timeframe_insert_weekly.sql
@sql/lookups/013_dim_timeframe_insert_monthly.sql
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create conditional dimension table setup script</name>
  <files>src/ta_lab2/scripts/setup/ensure_dim_tables.py, src/ta_lab2/scripts/setup/__init__.py</files>
  <action>
Create src/ta_lab2/scripts/setup/__init__.py (empty package file).

Create src/ta_lab2/scripts/setup/ensure_dim_tables.py with conditional table creation:
...
1. **table_exists(engine, schema, table_name)** -> bool:
   - Query information_schema.tables to check if table exists
   - Return True/False

2. **execute_sql_file(engine, filepath)** -> None:
   - Read SQL file content
   - Execute using engine.execute(text(content))
   - Log execution

3. **ensure_dim_timeframe(engine, sql_dir)** -> dict:
   - Check if dim_timeframe exists
   - If missing:
     - Execute 010_dim_timeframe_create.sql
     - Execute 011_dim_timeframe_insert_daily.sql
     - Execute 012_dim_timeframe_insert_weekly.sql
     - Execute 013_dim_timeframe_insert_monthly.sql
     - Execute 014_dim_timeframe_insert_yearly.sql (if exists)
   - Return {"existed": bool, "created": bool, "rows": int}

4. **ensure_dim_sessions(engine, sql_dir)** -> dict:
   - Check if dim_sessions exists
   - If missing:
     - Create table with schema matching dim_sessions.py (SessionKey + SessionMeta fields):
       - asset_class, region, venue, asset_key_type, asset_key, session_type (PK composite)
       - asset_id, timezone, session_open_local, session_close_local, is_24h
     - Insert default crypto session: ('CRYPTO', 'GLOBAL', 'DEFAULT', 'symbol', '*', 'RTH', NULL, 'UTC', '00:00', '23:59', TRUE)
     - Insert US equity session: ('EQUITY', 'US', 'NYSE', 'symbol', '*', 'RTH', NULL, 'America/New_York', '09:30', '16:00', FALSE)
   - Return {"existed": bool, "created": bool, "rows": int}

5. **main()** CLI:
   - Parse args: --sql-dir (default: sql/lookups), --dry-run
   - Run ensure_dim_timeframe
   - Run ensure_dim_sessions
   - Print summary: which tables existed, which were created, row counts
   - Exit 0 on success

**SQL_DIR default:** Look for sql/ directory relative to project root.

Pattern: Use `from ta_lab2.config import resolve_db_url` for database URL.
  </action>
  <verify>
python -c "from ta_lab2.scripts.setup.ensure_dim_tables import table_exists, ensure_dim_timeframe, ensure_dim_sessions; print('imports OK')"
  </verify>
  <done>
Conditional setup script created that checks table existence before creation, uses existing SQL seed files for dim_timeframe, creates dim_sessions inline
  </done>
</task>

<task type="auto">
  <name>Task 2: Create tests/time package and dim_timeframe validation tests</name>
...
<task type="auto">
  <name>Task 3: Create dim_sessions validation tests</name>
...
4. python -m ta_lab2.scripts.setup.ensure_dim_tables --dry-run shows what would be created
</verification>
...
- ensure_dim_tables.py creates missing tables using SQL seed files
- All tests pass when TARGET_DB_URL is set
- Tests skip gracefully when TARGET_DB_URL is not set
- If tables missing, running ensure_dim_tables.py creates them
</success_criteria>
