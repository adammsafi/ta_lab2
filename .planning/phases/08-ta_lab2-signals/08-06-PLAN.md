...
<task type="auto">
  ...
  <action>
    ...
    def main():
        ...
        if not args.validate_only:
            ...
            if failed:
                logger.error(f"{len(failed)} signal type(s) failed")
                return 1
  ...
  <verify>
    ...
    Verify help text shows --full-refresh, --validate-only, --skip-validation flags.
  </verify>
  <done>
    Orchestrated pipeline with parallel execution and reproducibility validation.
  </done>
</task>
...
<task type="auto">
  ...
  <done>
    20+ tests for reproducibility validation and pipeline integration.
  </done>
</task>
...
4. CLI supports various modes (full-refresh, validate-only, skip-validation)
</success_criteria>
