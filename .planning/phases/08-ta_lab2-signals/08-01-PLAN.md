...
<task type="auto">
  <name>Task 1a: Create dim_signals and state table DDLs</name>
  <files>
    sql/lookups/030_dim_signals.sql
    sql/signals/063_cmc_signal_state.sql
  </files>
  <action>
    Create DDL files for configuration and state tables:
  ...
  <verify>
    ...
    Run: `psql $TARGET_DB_URL -f sql/signals/063_cmc_signal_state.sql`
    Verify tables created with: `\dt public.dim_signals` and `\dt public.cmc_signal_state`
  </verify>
  <done>
    dim_signals exists with seed data (6 records), cmc_signal_state table created.
  </done>
</task>

<task type="auto">
  <name>Task 1b: Create signal tables DDLs</name>
  <files>
    sql/signals/060_cmc_signals_ema_crossover.sql
    sql/signals/061_cmc_signals_rsi_mean_revert.sql
    sql/signals/062_cmc_signals_atr_breakout.sql
  ...
  <action>
    Create DDL files for signal storage tables:
  ...
  <verify>
    ...
    Run: `psql $TARGET_DB_URL -f sql/signals/061_cmc_signals_rsi_mean_revert.sql`
    Run: `psql $TARGET_DB_URL -f sql/signals/062_cmc_signals_atr_breakout.sql`
    Verify tables created with: `\dt public.cmc_signals_*`
  </verify>
  <done>
    All 3 signal tables created with correct schema and indexes.
  </done>
</task>
