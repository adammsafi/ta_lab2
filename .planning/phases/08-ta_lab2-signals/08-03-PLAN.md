...
must_haves:
  truths:
    ...
    - "Adaptive threshold support built and callable via --adaptive CLI flag"
  artifacts:
    - path: "src/ta_lab2/scripts/signals/generate_signals_rsi.py"
      provides: "RSI mean reversion signal generation"
      exports: ["RSISignalGenerator", "compute_adaptive_thresholds"]
...
<task type="auto">
  ...
  <action>
    ...
    5. **Adaptive threshold support (wired to CLI):**
       ```python
       def compute_adaptive_thresholds(
           ...
           Called when --adaptive flag is passed to CLI.
    ...
    class RSISignalGenerator:
        ...
        def generate_for_ids(
            ...
            use_adaptive: bool = False,  # When True, use compute_adaptive_thresholds
        ) -> int:
            # Same pattern as EMA
            # Adds: rsi_at_entry, rsi_at_exit to records
            # If use_adaptive: compute rolling thresholds instead of static
    ```
  </action>
  <verify>
    Run: `python -c "from ta_lab2.scripts.signals.generate_signals_rsi import RSISignalGenerator, compute_adaptive_thresholds; print('OK')"`
    Verify class and function import without error.
  </verify>
  <done>
    RSISignalGenerator class created with generate_for_ids method, RSI value tracking at entry/exit, and adaptive threshold utility function exported and callable.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    def main():
        ...
        parser.add_argument('--adaptive', action='store_true',
                            help='Use adaptive rolling thresholds instead of static from dim_signals')
  ...
  <verify>
    ...
    Verify help text shows all arguments including --adaptive flag.
  </verify>
  <done>
    CLI script created with same flags as EMA refresh script plus --adaptive flag for adaptive thresholds.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    **Unit tests:**
    ...
    - test_use_adaptive_flag_calls_compute_adaptive: use_adaptive=True triggers adaptive computation
  ...
  <done>
    12+ tests passing for RSI signal generation including adaptive threshold tests.
  </done>
</task>
...
- [ ] compute_adaptive_thresholds utility function works and is exported
- [ ] CLI --adaptive flag triggers adaptive threshold computation
...
4. Adaptive threshold utility available and wired to CLI --adaptive flag
</success_criteria>
