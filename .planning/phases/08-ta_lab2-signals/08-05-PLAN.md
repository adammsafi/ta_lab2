...
<task type="auto">
  ...
  <action>
    ...
    class SignalBacktester:
        ...
        def save_backtest_results(self, result: BacktestResult) -> str:
            ...
            1. Generate run_id using uuid.uuid4() if not already set
            2. Use single transaction for atomicity (BEGIN/COMMIT)
            3. INSERT into cmc_backtest_runs first (parent)
            4. INSERT into cmc_backtest_trades (batch via executemany or to_sql)
            5. INSERT into cmc_backtest_metrics (single row)
            6. Handle conflicts: ON CONFLICT (run_id) DO UPDATE for reruns
            7. Return run_id for reference

            SQL template for runs:
            INSERT INTO cmc_backtest_runs
                (run_id, signal_type, signal_id, asset_id, start_ts, end_ts,
                 ...
                 signal_version, vbt_version, total_return, sharpe_ratio,
            ...
            with self.engine.begin() as conn:
                ...
                """), {...})
                # Batch insert trades
                result.trades_df['run_id'] = run_id
                result.trades_df.to_sql('cmc_backtest_trades', conn,
  ...
  <verify>
    Run: `python -c "from ta_lab2.backtests.vbt_runner import run_vbt_on_split; from ta_lab2.backtests.costs import CostModel; print('OK')"`
    Verify vbt infrastructure imports work.
    Run: `python -c "from ta_lab2.scripts.backtests import SignalBacktester, BacktestResult; print('OK')"`
    Verify class imports work.
  </verify>
  <done>
    SignalBacktester class created with run_backtest, save_backtest_results (with transaction handling, conflict resolution, run_id generation), and comprehensive metric extraction.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    **test_backtest_from_signals.py:**
    ...
    - test_save_backtest_results_returns_run_id: Verify UUID returned
    - test_save_backtest_results_transaction_atomic: All inserts in single transaction
...
- [ ] vbt_runner.run_vbt_on_split and CostModel are importable
...
- [ ] Results can be saved to database with atomic transactions
