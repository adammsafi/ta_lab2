...
Purpose: Per CONTEXT.md, validation must prove that memory queries can answer "What files exist in directory X?" for all 5 directories (ta_lab2 + 4 external). This is query-based validation, not file counting - we must prove memories are actually queryable.
...
Dry check: `python src/ta_lab2/tools/ai_orchestrator/memory/snapshot/validate_coverage.py --help` (if argparse used)
  ...
  <done>
validate_coverage.py exports validate_memory_coverage, run_validation. Script tests all required query types.
  </done>
</task>
...
3. Verify coverage_report.json created with:
   ...
   - Directory inventory queries work for all 4+ directories
...
6. Final verification - test the key Success Criteria query:
   ...
   for dir_name in ["ta_lab2", "Data_Tools", "fredtools2", "fedtools2"]:
...
Check success flag: `python -c "import json; r = json.load(open('.planning/phases/11-memory-preparation/validation/coverage_report.json')); print(f'Coverage: {r.get(\"overall_coverage_percentage\", 0)}%, Success: {r.get(\"success\", False)}')"`
  ...
  <done>
coverage_report.json exists with validation results. overall_coverage_percentage documented. Query "What files exist in directory X?" works for all indexed directories.
  </done>
</task>
...
- Note: ProjectTT not found at expected location (documented, not an error)
...
2. External directories captured with pre_integration_v0.5.0 tag
...
Values filled in (not placeholder text).
  ...
  <done>
MEMORY_STATE.md documents complete Phase 11 memory state with actual statistics. All requirements (MEMO-10, MEMO-11, MEMO-12) status documented.
  </done>
</task>
...
4. Query "What files exist in directory X?" works for all 4 directories
5. All three snapshot manifests referenced and validated
</verification>

<success_criteria>
- validate_coverage.py can run query-based validation on all directories
