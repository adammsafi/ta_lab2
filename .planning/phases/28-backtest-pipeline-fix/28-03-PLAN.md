...
<task type="auto">
  <name>Task 1: Run signal generators and verify signal table population</name>
  <files></files>
  <action>
Run each signal generator against the live database and verify signals are written successfully.

Step 1: Check dim_signals has active signal configurations:
```bash
python -c "
...
If no active signal configs exist in dim_signals, that is an infrastructure gap -- log it and note it in the summary, but it is a data seeding issue not a code bug.
  </action>
  <verify>
...
At least one signal table has closed positions (needed for backtesting).
  ...
  <done>Signal generators write to database without serialization errors. At least one signal table has closed positions ready for backtesting.</done>
</task>
...
5. Fixed _build_portfolio: tz-strip before vectorbt
