...
Purpose: Validate that subsystem pairs work together correctly before testing full E2E workflows. These tests use observability tracing for correlation ID propagation.
...
<task type="auto">
  ...
  <action>
    ...
    IMPORTANT: Verify tests import TracingContext for correlation ID tracking.
  ...
  <done>
    Orchestrator-memory tests cover context retrieval, prompt injection, result storage, and handoff operations. Tests import and use TracingContext for correlation.
  </done>
</task>
...
<task type="auto">
  ...
  <done>
    Orchestrator-ta_lab2 tests cover feature refresh, validation, signal generation, backtest, and tracing propagation. Tests import and use TracingContext for correlation.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    IMPORTANT: Verify tests import TracingContext for correlation ID tracking in failure scenarios.
  ...
  <done>
    Failure scenario tests cover all four categories: unavailable, partial, timeout, invalid state. Tests import and use TracingContext for correlation. Includes --fail-fast behavior.
  </done>
</task>
...
# Verify TracingContext imports (key_link verification)
grep -l "from ta_lab2.observability" tests/integration/test_*.py
...
- All test files import TracingContext from ta_lab2.observability (verified via grep)
