...
must_haves:
  truths:
    - "pytest markers @real_deps, @mixed_deps, @mocked_deps classify tests by dependency requirements"
    - "Integration tests skip gracefully when real infrastructure unavailable"
    - "Shared fixtures provide database, memory client, and test data"
  artifacts:
...
Purpose: Enable flexible test execution - real infrastructure tests for validation, mocked tests for CI/CD, mixed for development.
