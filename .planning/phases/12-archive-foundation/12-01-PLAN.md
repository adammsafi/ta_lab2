---
phase: 12-archive-foundation
plan: 01
type: execute
wave: 1
depends_on: []
files_modified:
  - .archive/00-README.md
  - .archive/deprecated/2026-02-02/.gitkeep
  - .archive/deprecated/manifest.json
  - .archive/refactored/2026-02-02/.gitkeep
  - .archive/refactored/manifest.json
  - .archive/migrated/2026-02-02/.gitkeep
  - .archive/migrated/manifest.json
  - .archive/documentation/2026-02-02/.gitkeep
  - .archive/documentation/manifest.json
  - .archive/test_git_history/.gitkeep
autonomous: true

must_haves:
  truths:
    - ".archive/ directory exists with category-first structure"
    - "Category subdirectories (deprecated, refactored, migrated, documentation) exist"
    - "Each category has dated subdirectories and manifest.json"
    - "00-README.md documents archive structure and NO DELETION policy"
    - "git mv preserves file history (git log --follow shows full history)"
    - "Verification pattern documented for future archive operations"
  artifacts:
    - path: ".archive/00-README.md"
      provides: "Archive documentation with structure, purpose"
      min_lines: 50
    - path: ".archive/deprecated/2026-02-02/.gitkeep"
      provides: "Placeholder for deprecated category date subdirectory"
    - path: ".archive/refactored/2026-02-02/.gitkeep"
      provides: "Placeholder for refactored category date subdirectory"
    - path: ".archive/migrated/2026-02-02/.gitkeep"
      provides: "Placeholder for migrated category date subdirectory"
    - path: ".archive/documentation/2026-02-02/.gitkeep"
      provides: "Placeholder for documentation category date subdirectory"
    - path: ".archive/deprecated/manifest.json"
      provides: "Template manifest for deprecated category"
      contains: "$schema"
    - path: ".archive/test_git_history/test_file.py"
      provides: "Permanent test artifact demonstrating history preservation"
  key_links:
    - from: ".archive/00-README.md"
      to: "archive categories"
      via: "directory structure documentation"
      pattern: "deprecated|refactored|migrated|documentation"
    - from: ".archive/deprecated/manifest.json"
      to: "archive schema"
      via: "$schema field"
      pattern: "ta_lab2.local/schemas/archive-manifest"
---

<objective>
Create the .archive/ directory structure and verify git history preservation patterns.

Purpose: Establish the foundation for safe file archiving before any v0.5.0 reorganization begins. This ensures all future archive operations have a documented home and proven git history preservation patterns.

Output:
- .archive/ directory with category-first structure (per CONTEXT.md)
- 00-README.md documenting archive structure
- manifest.json template for each category
- Verified git mv workflow that preserves file history
</objective>

<execution_context>
@./.claude/get-shit-done/workflows/execute-plan.md
@./.claude/get-shit-done/templates/summary.md
</execution_context>

<context>
@.planning/PROJECT.md
@.planning/ROADMAP.md
@.planning/STATE.md
@.planning/phases/12-archive-foundation/12-CONTEXT.md
@.planning/phases/12-archive-foundation/12-RESEARCH.md

# Relevant prior work - memory/migration.py patterns
@src/ta_lab2/tools/ai_orchestrator/memory/migration.py
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create archive directory structure with README and manifest templates</name>
  <files>
    .archive/00-README.md
    .archive/deprecated/2026-02-02/.gitkeep
    .archive/deprecated/manifest.json
    .archive/refactored/2026-02-02/.gitkeep
    .archive/refactored/manifest.json
    .archive/migrated/2026-02-02/.gitkeep
    .archive/migrated/manifest.json
    .archive/documentation/2026-02-02/.gitkeep
    .archive/documentation/manifest.json
  </files>
  <action>
Create the .archive/ directory structure following the category-first pattern from CONTEXT.md:

1. Create .archive/ root directory

2. Create 00-README.md with:
   - Purpose: "Archive for files removed from active codebase during v0.5.0 reorganization"
   - Structure diagram showing:
     ```
     .archive/
     ├── 00-README.md           # This file
     ├── deprecated/            # Code no longer needed
     │   ├── manifest.json      # File inventory for this category
     │   └── YYYY-MM-DD/        # Date subdirectories
     │       └── *.py           # Archived files
     ├── refactored/            # Code replaced during refactor
     │   ├── manifest.json
     │   └── YYYY-MM-DD/
     ├── migrated/              # Code moved to new location
     │   ├── manifest.json
     │   └── YYYY-MM-DD/
     └── documentation/         # Archived docs (Word, Excel, etc.)
         ├── manifest.json
         └── YYYY-MM-DD/
     ```
   - Category definitions:
     - deprecated: Code no longer needed (backup artifacts, old implementations)
     - refactored: Code replaced by refactored version (*_refactored.py originals)
     - migrated: Code moved to new location within ta_lab2
     - documentation: Non-code files (Word docs, Excel, etc.)
   - NO DELETION policy: "Files in .archive/ are preserved in git history. Never use rm -rf on this directory."
   - Structure note: "Category-first organization for easy browsing by type"

3. Create category subdirectories with date folders:
   ```bash
   mkdir -p .archive/deprecated/2026-02-02
   mkdir -p .archive/refactored/2026-02-02
   mkdir -p .archive/migrated/2026-02-02
   mkdir -p .archive/documentation/2026-02-02
   ```

4. Create .gitkeep files in each date directory to preserve empty directories in git

5. Create manifest.json template in each category directory:
   ```json
   {
     "$schema": "https://ta_lab2.local/schemas/archive-manifest/v1.0.0",
     "version": "1.0.0",
     "category": "<deprecated|refactored|migrated|documentation>",
     "created_at": "2026-02-02T<timestamp>Z",
     "total_files": 0,
     "total_size_bytes": 0,
     "files": []
   }
   ```
   Create one for each category: deprecated, refactored, migrated, documentation

The .gitkeep files ensure empty directories are tracked by git.
Manifests will be populated by Plan 12-02's functions as files are archived.
  </action>
  <verify>
Check structure: `ls -la .archive/`
Check categories: `ls -la .archive/deprecated/ .archive/refactored/ .archive/migrated/ .archive/documentation/`
Check README: `cat .archive/00-README.md | head -20`
Check manifests: `cat .archive/deprecated/manifest.json`
Verify manifest has $schema field: `grep -q "\$schema" .archive/deprecated/manifest.json && echo "Schema present"`
  </verify>
  <done>
.archive/ directory created with category-first structure. 00-README.md documents structure and NO DELETION policy. Each category (deprecated, refactored, migrated, documentation) has: 2026-02-02 date subdirectory with .gitkeep, and manifest.json template with schema versioning.
  </done>
</task>

<task type="auto">
  <name>Task 2: Verify git mv history preservation pattern</name>
  <files>
    .archive/test_git_history/.gitkeep
    .archive/test_git_history/test_file.py
  </files>
  <action>
Create a test file, archive it using git mv, and verify git log --follow preserves history:

1. Create a test file in project root:
   ```bash
   echo "# Test file for git history verification" > test_archive_history.py
   echo "def test_function():" >> test_archive_history.py
   echo "    return 'original'" >> test_archive_history.py
   git add test_archive_history.py
   git commit -m "test(12-01): create test file for git history verification"
   ```

2. Create .archive/test_git_history/ directory for test files

3. Archive using git mv (pure move, no content changes):
   ```bash
   git mv test_archive_history.py .archive/test_git_history/test_file.py
   git commit -m "test(12-01): archive test file using git mv"
   ```

4. Verify history preservation:
   ```bash
   git log --follow --oneline .archive/test_git_history/test_file.py
   ```
   Expected output: 2+ commits visible (the move commit AND the original creation commit)

5. Document verification in 00-README.md:
   - Add section "## History Verification"
   - Include the verification command and expected output
   - Note: "If git log --follow shows only 1 commit, the move was NOT pure"

IMPORTANT: The test file should remain in .archive/test_git_history/ as a permanent verification artifact. Do NOT delete it after the test.
  </action>
  <verify>
Run: `git log --follow --oneline .archive/test_git_history/test_file.py`
Expected: At least 2 commits visible (original creation + move)

Run: `grep -A 5 "History Verification" .archive/00-README.md`
Expected: Section exists with verification instructions
  </verify>
  <done>
- Test file created, committed, moved via git mv, committed again
- git log --follow shows 2+ commits (history preserved through move)
- 00-README.md updated with History Verification section
- Test file remains as permanent verification artifact in .archive/test_git_history/
  </done>
</task>

</tasks>

<verification>
Phase 12 Plan 01 verification (run all):

1. Structure verification:
   ```bash
   ls -la .archive/
   test -d .archive/deprecated && test -d .archive/refactored && test -d .archive/migrated && test -d .archive/documentation && echo "Categories OK"
   test -d .archive/deprecated/2026-02-02 && echo "Date structure OK"
   cat .archive/00-README.md | wc -l
   ```
   Expected: All directories exist, README has 50+ lines

2. Manifest template verification:
   ```bash
   cat .archive/deprecated/manifest.json
   grep -c '$schema\|version\|category\|files' .archive/deprecated/manifest.json
   ```
   Expected: Valid JSON with $schema, version, category, and files fields (4+ matches)

3. All category manifests exist:
   ```bash
   test -f .archive/deprecated/manifest.json && test -f .archive/refactored/manifest.json && test -f .archive/migrated/manifest.json && test -f .archive/documentation/manifest.json && echo "All manifests OK"
   ```

4. Git history verification:
   ```bash
   git log --follow --oneline .archive/test_git_history/test_file.py | wc -l
   ```
   Expected: 2 or more commits

5. README content verification:
   ```bash
   grep -c "deprecated\|refactored\|migrated\|documentation" .archive/00-README.md
   ```
   Expected: 4+ matches (all categories documented)

6. NO DELETION policy documented:
   ```bash
   grep -i "never.*rm\|preserved" .archive/00-README.md
   ```
   Expected: At least 1 match
</verification>

<success_criteria>
- .archive/ directory exists with category-first structure
- Category subdirectories exist: deprecated, refactored, migrated, documentation
- Each category has: manifest.json and 2026-02-02 date subdirectory
- 00-README.md documents category-first structure, NO DELETION policy, and category definitions
- Category manifests have $schema versioning and empty files array
- Git mv verified to preserve history (test file shows 2+ commits via git log --follow)
- Verification pattern established for future archive operations
</success_criteria>

<output>
After completion, create `.planning/phases/12-archive-foundation/12-01-SUMMARY.md`
</output>
