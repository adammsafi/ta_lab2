...
<task type="auto">
  <name>Task 3: Update database_utils __init__.py and verify cross-imports</name>
...
- Example: `from ta_lab2.tools.data_tools.database_utils.write_daily_emas import ...`
- NOT: `import write_daily_emas` or bare module imports with sys.path hacks

Verify cross-imports use ta_lab2 paths:
```bash
# Should find ta_lab2 imports only (no bare module imports for migrated scripts)
grep -E "^(from|import)" src/ta_lab2/tools/data_tools/database_utils/*.py | grep -v "ta_lab2" | grep -v "^#" | grep -v "pathlib\|logging\|argparse\|psycopg2\|pandas\|numpy\|typing\|os\|sys"
```
...
```
  ...
  <verify>
    - `python -c "from ta_lab2.tools.data_tools import database_utils; print(database_utils.__all__)"` succeeds
    - No bare imports of other migrated scripts (only ta_lab2 import paths)
  </verify>
  <done>database_utils module exports all EMA utilities; imports work from package level; cross-imports use ta_lab2 paths</done>
</task>
...
6. Any cross-script imports use ta_lab2 import paths (not relative filesystem)
</success_criteria>
