...
2. Check if it duplicates ta_lab2/tools/ai_orchestrator/memory/snapshot/extract_codebase.py
3. If unique functionality, migrate; if duplicate, consider archiving or merging
...
1. Mem0 setup utilities
2. Check if these duplicate ta_lab2/tools/ai_orchestrator/memory/mem0_config.py
3. If unique, migrate; if duplicate, archive with note pointing to existing
...
- Handle optional dependencies gracefully
  ...
  <done>Memory generation and bank tools migrated</done>
</task>

<task type="auto">
  <name>Task 3: Update memory __init__.py with exports</name>
...
Adjust based on actual functions found in migrated scripts.
...
```
  ...
  <verify>
    python -c "from ta_lab2.tools.data_tools import memory; print(memory.__all__)"
  </verify>
  <done>memory module exports core functions; optional deps handled gracefully</done>
</task>
...
2. Memory generation tools migrated
3. REST API (memory_bank_rest) migrated with flask handling
4. Mem0 setup utilities migrated
