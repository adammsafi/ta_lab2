...
<task type="auto">
  <name>Task 3: Update analysis __init__.py with exports and verify cross-imports</name>
...
- Example: `from ta_lab2.tools.data_tools.analysis.tree_structure import ...`
- NOT: `import tree_structure` or `from . import tree_structure` with sys.path hacks

Verify cross-imports use ta_lab2 paths:
```bash
# Should find ta_lab2 imports only (no bare module imports for migrated scripts)
grep -E "^(from|import)" src/ta_lab2/tools/data_tools/analysis/*.py | grep -v "ta_lab2" | grep -v "^#" | grep -v "pathlib\|logging\|argparse\|ast\|csv\|json\|os\|sys\|typing"
```
...
```
  ...
  <verify>
    - `python -c "from ta_lab2.tools.data_tools import analysis; print(analysis.__all__)"` succeeds
    - No bare imports of other migrated scripts (only ta_lab2 import paths)
  </verify>
  <done>analysis module exports public functions; imports work from package level; cross-imports use ta_lab2 paths</done>
</task>
...
7. Any cross-script imports use ta_lab2 import paths (not relative filesystem)
</success_criteria>
