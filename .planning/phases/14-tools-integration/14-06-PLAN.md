...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/ta_lab2/tools/data_tools/export/__init__.py"
      ...
      contains: "__all__"
  key_links:
    - from: "src/ta_lab2/tools/data_tools/export/chatgpt_pipeline.py"
      to: "export_chatgpt_conversations.py"
      via: "ta_lab2 import"
      pattern: "from ta_lab2.tools.data_tools.export"
---
...
2. **CRITICAL: Update internal imports to use ta_lab2 paths:**
   - Change: `from export_chatgpt_conversations import ...`
   - To: `from ta_lab2.tools.data_tools.export.export_chatgpt_conversations import ...`
   - Same for any other migrated script references

3. Check for ALL internal script references and update to ta_lab2 paths:
   ```bash
   # Find all imports in original file that reference other scripts
   grep -E "^(from|import)" C:\Users\asafi\Downloads\Data_Tools\chatgpt\chatgpt_pipeline.py
   ```
   Each one that references another Data_Tools script must become a ta_lab2 import.
...
- Use module-level logger
  ...
  <done>Diff and cleaning tools migrated with cross-script imports updated to ta_lab2 paths</done>
</task>

<task type="auto">
  <name>Task 3: Migrate processing tools, update __init__.py, and verify cross-imports</name>
  <files>
    src/ta_lab2/tools/data_tools/export/process_new_chatgpt_dump.py
...
**Cross-import verification (CRITICAL for this module):**
The chatgpt_pipeline.py and other orchestrator scripts likely reference each other.
...
grep -E "^(from|import)" src/ta_lab2/tools/data_tools/export/*.py | grep -v "ta_lab2" | grep -v "^#" | grep -v "pathlib\|logging\|argparse\|json\|csv\|zipfile\|typing\|os\|sys\|re\|datetime"
```

**If bare imports found**, update them:
- `import chatgpt_export_diff` -> `from ta_lab2.tools.data_tools.export import chatgpt_export_diff`
- `from chatgpt_export_clean import clean` -> `from ta_lab2.tools.data_tools.export.chatgpt_export_clean import clean`
...
- All cross-script imports updated to ta_lab2 paths
```
  </action>
  <verify>
    - `python -c "from ta_lab2.tools.data_tools import export; print(export.__all__)"` succeeds
    - `grep -E "^(from|import).*chatgpt" src/ta_lab2/tools/data_tools/export/*.py | grep -v ta_lab2` returns nothing (all internal refs use ta_lab2 paths)
  </verify>
  <done>export module complete with all processing tools; __init__.py exports public functions; all cross-imports use ta_lab2 paths</done>
</task>
...
- Cross-script references use `from ta_lab2.tools.data_tools.export...` import paths (no bare imports)
</verification>
...
8. All cross-script imports use ta_lab2 import paths (not relative filesystem or bare module imports)
</success_criteria>
