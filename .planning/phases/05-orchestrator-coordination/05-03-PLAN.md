...
wave: 2
depends_on: ["05-02"]
# Also requires Phase 2 (memory integration) to be complete
# Uses memory/update.add_memory and memory/query.get_memory_by_id
files_modified:
...
<task type="auto">
  <name>Task 0: Verify Phase 2 memory integration is operational</name>
  <files>None (verification only)</files>
  <action>
Verify Phase 2 memory modules are operational before handoff integration:

1. Check memory modules exist and import successfully
2. Verify memory client can connect to ChromaDB/Qdrant
3. Test basic add_memory and get_memory_by_id operations

Commands:
```bash
# Test memory module imports
python -c "from ta_lab2.tools.ai_orchestrator.memory.update import add_memory; print('memory.update available')"
python -c "from ta_lab2.tools.ai_orchestrator.memory.query import get_memory_by_id; print('memory.query available')"

# Test memory client connection (from Phase 2)
python -c "from ta_lab2.tools.ai_orchestrator.memory.client import MemoryClient; client = MemoryClient(); print('MemoryClient operational')"
```
  </action>
  <verify>
All three commands succeed without errors
  ...
  <done>
Phase 2 memory integration verified - handoff.py can safely import and use memory modules
  </done>
</task>
...
<verification>
1. Phase 2 memory pre-check: Run Task 0 verification commands to confirm memory modules are operational
2. Memory module verification: `python -c "from ta_lab2.tools.ai_orchestrator.memory.update import add_memory; from ta_lab2.tools.ai_orchestrator.memory.query import get_memory_by_id; print('Memory modules available')"`
3. Import verification: `python -c "from ta_lab2.tools.ai_orchestrator.handoff import spawn_child_task, load_handoff_context"`
4. Test suite: `pytest tests/orchestrator/test_handoff.py -v`
5. All orchestrator tests pass: `pytest tests/orchestrator/ -v`
</verification>

<success_criteria>
- Phase 2 memory integration verified operational before implementation begins
