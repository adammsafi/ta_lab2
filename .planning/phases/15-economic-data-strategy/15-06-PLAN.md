...
user_setup:
  - service: qdrant
    why: "Vector store for Mem0 memory storage"
    prereq:
      - task: "Ensure Qdrant is running"
        command: "start_qdrant_server.bat OR docker run -p 6333:6333 qdrant/qdrant"
        verify: "curl http://localhost:6333/health returns OK"
  - service: openai
    why: "LLM and embeddings for Mem0"
    env_vars:
      - name: OPENAI_API_KEY
        source: "openai_config.env file (already exists in project root)"
        verify: "File openai_config.env exists with OPENAI_API_KEY=sk-..."
...
1. **Qdrant is running** on localhost:6333
   - Start via: `start_qdrant_server.bat` (Windows) or `docker run -p 6333:6333 qdrant/qdrant`
   - Verify: `curl http://localhost:6333/health` returns healthy status

2. **OpenAI API key is configured** in openai_config.env
   - File should exist at project root with `OPENAI_API_KEY=sk-...`
   - Script loads this via python-dotenv

If either prerequisite is missing, the script will fail with connection errors.
</runtime_prerequisites>
...
<task type="auto">
  ...
  <files>scripts/update_phase15_memory.py</files>
  <action>
