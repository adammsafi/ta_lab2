...
must_haves:
  truths:
    ...
    - "FredProvider uses all reliability features when fetching data"
  artifacts:
  ...
  key_links:
    - from: "fred_provider.py"
      to: "rate_limiter.py, cache.py, circuit_breaker.py, quality.py"
      via: "composition in __init__ and usage in get_series"
      pattern: "self._rate_limiter.*self._cache.*self._circuit_breaker.*self._validator"
---
...
<task type="auto">
  <name>Task 4: Integrate reliability features into FredProvider</name>
  <files>src/ta_lab2/integrations/economic/fred_provider.py</files>
  <action>
Update FredProvider to instantiate and use all four reliability components.

1. Add imports at top of fred_provider.py:
```python
...
2. Update FredProvider.__init__ to instantiate components:
```python
def __init__(self, api_key: Optional[str] = None):
    """Initialize FredProvider with reliability features.
...
3. Update get_series method to use reliability features in sequence:
```python
def get_series(
    self,
    series_id: str,
...
4. Add quality_report field to FetchResult if not present:
```python
@dataclass
class FetchResult:
    """Result of a data fetch operation."""
    success: bool
    data: Optional[EconomicSeries] = None
...
5. Add convenience methods for accessing reliability component stats:
```python
...
```bash
python -c "
from ta_lab2.integrations.economic.fred_provider import FredProvider
p = FredProvider()
# Verify components are instantiated
assert hasattr(p, '_rate_limiter'), 'Missing _rate_limiter'
assert hasattr(p, '_cache'), 'Missing _cache'
assert hasattr(p, '_circuit_breaker'), 'Missing _circuit_breaker'
assert hasattr(p, '_validator'), 'Missing _validator'
print('All reliability components instantiated')
# Verify stats method exists
stats = p.get_reliability_stats()
print(f'Reliability stats: {stats}')
"
```
  </verify>
  <done>FredProvider integrates rate limiter, cache, circuit breaker, and quality validator. get_series uses all four in sequence: cache check -> rate limit -> circuit breaker API call -> quality validation -> cache result.</done>
</task>
...
8. FredProvider.__init__ instantiates all four reliability components
9. FredProvider.get_series uses components in sequence: cache -> rate limit -> circuit breaker -> validate -> cache
10. FredProvider.get_reliability_stats() provides visibility into component state
</verification>
...
- FredProvider uses all four components when fetching data
- Reliability component stats accessible via get_reliability_stats()
</success_criteria>
