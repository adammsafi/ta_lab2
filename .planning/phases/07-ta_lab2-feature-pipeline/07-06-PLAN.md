...
must_haves:
  truths:
    ...
    - "Unified view refresh fails gracefully when source tables missing or empty"
  artifacts:
    ...
    **DailyFeaturesStore:**
    ...
    class DailyFeaturesStore:
        ...
        def check_source_tables_exist(self) -> dict[str, bool]:
            ...
            Returns dict mapping table_name -> exists_with_data.
        ...
        def refresh_for_ids(
            ...
            2. If full_refresh: DELETE WHERE id IN ids
            3. Else: DELETE WHERE id IN ids AND ts >= dirty_start
        ...
        def _build_join_query(self, ids: list[int], start: str, end: str) -> str:
            ...
            Uses LEFT JOINs so missing source tables result in NULL columns,
            not failed queries.
            """
...
<task type="auto">
  ...
  <action>
    ...
    **test_daily_features_view.py tests:**
    ...
    13. test_graceful_missing_source_table - Continues when source missing
    14. test_graceful_empty_source - NULL columns when source empty
...
- [ ] Graceful handling when source tables missing or empty
- [ ] All 14 tests pass
</verification>
...
- Refresh fails gracefully (with warnings) when source tables missing/empty
</success_criteria>
