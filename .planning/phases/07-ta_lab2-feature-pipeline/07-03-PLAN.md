...
<task type="auto">
  ...
  <action>
    ...
    **IMPORTANT - Use existing returns.py functions:**
    ```python
    ...
    class ReturnsFeature(BaseFeature):
        ...
        def compute_features(self, df_source: pd.DataFrame) -> pd.DataFrame:
            ...
            2. Compute bar-to-bar returns using b2t_pct_delta and b2t_log_delta:
               - ret_1d_pct = b2t_pct_delta(df, 'close')
               - ret_1d_log = b2t_log_delta(df, 'close')
    ...
    **Important details:**
    - MUST import from ta_lab2.features.returns: b2t_pct_delta, b2t_log_delta
    - Use these existing functions for b2t calculations (do NOT reimplement)
  ...
  <action>
    **refresh_cmc_returns_daily.py CLI:**
    ...
    def main():
        ...
        state_manager = FeatureStateManager(engine, feature_type="returns")
...
- [ ] ReturnsFeature imports and uses b2t_pct_delta, b2t_log_delta from returns.py
