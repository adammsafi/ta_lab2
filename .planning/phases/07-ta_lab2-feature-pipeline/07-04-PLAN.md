...
<task type="auto">
  ...
  <action>
    ...
    **IMPORTANT - Import and use existing vol.py functions:**
    ```python
    ...
    class VolatilityFeature(BaseFeature):
        ...
        def compute_features(self, df_source: pd.DataFrame) -> pd.DataFrame:
            ...
            2. Compute Parkinson for each window:
               add_parkinson_vol(df, window=20, annualize=True)
               add_parkinson_vol(df, window=63, annualize=True)
               add_parkinson_vol(df, window=126, annualize=True)
            3. Compute Garman-Klass for each window:
               add_garman_klass_vol(df, window=20, annualize=True)
               add_garman_klass_vol(df, window=63, annualize=True)
               add_garman_klass_vol(df, window=126, annualize=True)
            4. Compute Rogers-Satchell for each window:
               add_rogers_satchell_vol(df, window=20, annualize=True)
               add_rogers_satchell_vol(df, window=63, annualize=True)
               add_rogers_satchell_vol(df, window=126, annualize=True)
            5. Compute ATR:
               add_atr(df, period=14)
            6. Compute rolling historical vol from returns:
               add_rolling_vol_from_returns_batch(df, windows=[20, 63, 126])
    ...
    **Important details:**
    - MUST import from ta_lab2.features.vol - do NOT duplicate calculation logic
    - Reuse ALL functions from vol.py with appropriate window parameters
  ...
  <action>
    **refresh_cmc_vol_daily.py CLI:**
    ...
    def main():
        ...
        state_manager = FeatureStateManager(engine, feature_type="vol")
...
- [ ] VolatilityFeature imports and uses vol.py functions (no duplication)
