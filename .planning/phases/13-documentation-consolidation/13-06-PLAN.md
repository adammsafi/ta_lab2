...
# MEMO-13 Coverage Note:
# MEMO-13 requires "file-level memory updates during reorganization."
# This plan satisfies MEMO-13 via BATCH memory creation after conversions complete.
# Rationale for batch approach (vs incremental during Plans 13-03/13-04):
# 1. Practicality: If conversions fail partway, batch avoids orphan memories
# 2. Idempotency: Batch with duplicate checking is cleaner than incremental updates
# 3. Context efficiency: One memory session vs multiple partial sessions
# 4. Phase 13 Context specified "maximum practical granularity" - batch is practical
# The batch approach creates the SAME memories that incremental would create,
# just deferred until all conversions succeed. MEMO-13 is fully satisfied.
---
...
NOTE: This plan executes in Wave 4 (after all conversions complete) to ensure
batch memory creation only happens for successfully converted documents. This
satisfies MEMO-13's "file-level memory updates during reorganization" requirement
via a practical batch approach rather than incremental updates.
</objective>
...
<task type="auto">
  <name>Task 2: Update memory with document conversions (MEMO-13)</name>
  <files>None (memory operations)</files>
  <action>
Execute batch memory updates for all converted documents.

This task satisfies MEMO-13 ("file-level memory updates during reorganization")
by creating memories for ALL successfully converted documents in a single batch.
...
"
  ...
  <done>
    - Memory updated with moved_to relationships for all converted docs (MEMO-13 satisfied)
...
<task type="auto">
  <name>Task 3: Create phase 13 memory snapshot (MEMO-14)</name>
  <files>None (memory operations)</files>
  <action>
...
"
  ...
  <done>
    - Phase 13 memory snapshot created (MEMO-14 satisfied)
...
- [ ] Memory contains moved_to relationships for converted documents (MEMO-13)
- [ ] Section-level memories created for major headings
- [ ] Phase 13 memory snapshot created with completion metrics (MEMO-14)
