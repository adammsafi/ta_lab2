...
<task type="auto">
  ...
  <action>
Move original ProjectTT files to .archive/documentation/ using git mv.

1. Create archive date directory:
   mkdir -p .archive/documentation/$(date +%Y-%m-%d)

2. For each converted file from inventory:
   - Use git mv to preserve history (Phase 12 pattern)
   - Move to .archive/documentation/YYYY-MM-DD/{filename}
...
6. Verify git history preservation:
   git log --follow --oneline .archive/documentation/YYYY-MM-DD/CoreComponents.docx

Important: Files are external to ta_lab2 repo, so:
- Copy files first (cp), then add to git
- Cannot use git mv across repos
- Use regular file copy + git add
  </action>
  <verify>
    ls .archive/documentation/2026-*/*.docx 2>/dev/null | head -5 || echo "Check archive paths"
    python -c "from ta_lab2.tools.archive.manifest import validate_manifest; valid, issues = validate_manifest(Path('.archive/documentation/manifest.json')); print('Valid' if valid else issues)"
  </verify>
  <done>
...
<success_criteria>
- [ ] All ProjectTT .docx and .xlsx files archived in .archive/documentation/
