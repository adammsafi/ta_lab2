...
<task type="auto">
  <name>Task 2: Execute metadata migration</name>
  <files>
    src/ta_lab2/tools/ai_orchestrator/memory/migration.py
  </files>
  <action>
    1. Run migration in dry-run mode first:
       ...
       Expected output:
       - Total: 3,763
       - Updated: ~3,763 (first run, all need metadata)
       - Skipped: 0
       - Errors: 0

    2. If dry-run looks correct, run actual migration:
       python -m ta_lab2.tools.ai_orchestrator.memory.migration

    3. Verify migration completed:
       - Check migration log output
       - No errors in error_ids

    4. Run validation to confirm:
       python -m ta_lab2.tools.ai_orchestrator.memory.migration --validate

    IMPORTANT: Migration should be idempotent. If something goes wrong, re-running should not corrupt data.
  </action>
  <verify>
    python -c "
...
"
  ...
  <done>
    Metadata migration executed successfully on all 3,763 memories
  </done>
</task>
