...
<task type="auto">
  <name>Task 1: Create dim_assets table DDL</name>
  <files>sql/ddl/create_dim_assets.sql</files>
  <action>
Create SQL DDL file that:
1. Creates `public.dim_assets` table from `dim_sessions` WHERE `asset_class = 'CRYPTO'`
2. Includes columns: id, asset_class, symbol (from dim_sessions)
3. Uses CREATE TABLE AS SELECT pattern (consistent with sql/snapshots/ examples)
4. Adds PRIMARY KEY on id
5. Includes IF NOT EXISTS guard for idempotence
...
AVOID: Creating a full dim_assets migration - this is just a simple filter table for baseline capture scope.
  ...
  <verify>cat sql/ddl/create_dim_assets.sql shows CREATE TABLE with CRYPTO filter</verify>
  <done>DDL file exists with correct pattern, ready to execute against database</done>
</task>
...
2. Python module structure exists: `test -d src/ta_lab2/scripts/baseline`
3. All imports work: `python -c "from ta_lab2.scripts.baseline import comparison_utils, metadata_tracker; print('OK')"`
4. Comparison handles NaN correctly: Write quick test with np.nan values
</verification>
