...
def _sample_tfs_and_ids(conn, bar_table: str) -> Tuple[List[str], List[int]]:
    ...
    if time_min:
        id_sql += f" WHERE \"timestamp\" >= '{time_min}'"
    id_sql += " ORDER BY id"
...
def _load_bars_polars(
    ...
    if time_min:
        where_clauses.append(f"\"timestamp\" >= '{time_min}'")
    if time_max:
        where_clauses.append(f"\"timestamp\" < '{time_max}'")
    ...
    sql = f"""
        ...
        FROM {bar_table}
        WHERE {where_clause}
        ORDER BY id, tf, bar_seq DESC, "timestamp" DESC
...
# -----------------------------------------------------------------------------
# Principle #2: Polars vectorized OHLC computation
# -----------------------------------------------------------------------------
def _compute_expected_ohlc_polars(
    ...
    # Join bars with daily data on id and time range
    joined = bars_df.join_asof(
        daily_df.rename({"ts": "daily_ts"}),
        left_on="time_close",
        right_on="daily_ts",
        by="id",
        strategy="backward",
    ...
    expected = joined.group_by(["id", "tf", "bar_seq", "time_close"]).agg(
        [
            ...
            # Time high: earliest occurrence of max(high)
            pl.when(pl.col("high") == pl.col("high").max())
            .then(pl.col("timehigh"))
            .min()
            .alias("time_high_exp"),
            # Time low: earliest occurrence of min(low)
            pl.when(pl.col("low") == pl.col("low").min())
            .then(pl.col("timelow"))
            .min()
            .alias("time_low_exp"),
            pl.count().alias("n_src"),
        ]
    )
