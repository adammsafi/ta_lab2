...
def test_realized_tf_days_ok_within_bounds(dim_tf):
    ...
    assert result is True, f"Expected {tf} with {mid_point} days (within bounds [{min_days}, {max_days}]) to return True"
...
def test_realized_tf_days_ok_out_of_bounds(dim_tf):
    ...
    assert result is False, f"Expected {tf} with 100 days (out of bounds) to return False"
...
def test_calendar_tf_has_anchor(dim_tf):
    ...
    calendar_tfs = list(dim_tf.list_tfs(alignment_type="calendar", canonical_only=False))

    assert len(calendar_tfs) > 0, "Expected at least one calendar-aligned TF in dim_timeframe"
    ...
    for tf in calendar_tfs:
        ...
        assert anchor is not None, f"Calendar TF {tf} has alignment_type='calendar' but calendar_anchor is None"
...
def test_tf_day_tf_no_anchor(dim_tf):
    ...
    assert len(tf_day_tfs) > 0, "Expected at least one tf_day-aligned TF in dim_timeframe"

    # Check each has no calendar_anchor
    for tf in tf_day_tfs:
        anchor = dim_tf.calendar_anchor(tf)
        assert anchor is None, f"TF_day TF {tf} has alignment_type='tf_day' but calendar_anchor is set: {anchor}"
...
def test_alignment_types_coverage(dim_tf):
    ...
    assert len(calendar_tfs) > 0, "Expected at least one TF with alignment_type='calendar'"
...
def test_tf_days_min_max_relationship(dim_tf):
    ...
    assert len(valid) > 0, "Expected at least some TFs with valid min/nominal/max relationship"
