...
class TestRSISignalTransformation(unittest.TestCase):
    ...
    def test_generate_loads_thresholds_from_config(self):
        ...
        with patch.object(generator, "load_features", return_value=test_features):
            with patch(
                "ta_lab2.scripts.signals.generate_signals_rsi.make_signals"
            ) as mock_make_signals:
...
class TestAdaptiveThresholds(unittest.TestCase):
    ...
    def test_adaptive_thresholds_computes_percentiles(self):
        ...
        df = pd.DataFrame({"rsi_14": [20, 30, 40, 50, 60, 70, 80, 30, 40, 50]})

        lower, upper = compute_adaptive_thresholds(
            df, rsi_col="rsi_14", lookback=5, lower_pct=20.0, upper_pct=80.0
        )
    ...
    def test_use_adaptive_flag_calls_compute_adaptive(self):
        ...
        with patch.object(generator, "load_features", return_value=test_features):
            with patch(
                "ta_lab2.scripts.signals.generate_signals_rsi.compute_adaptive_thresholds"
            ) as mock_adaptive:
                with patch(
                    "ta_lab2.scripts.signals.generate_signals_rsi.make_signals"
                ) as mock_make_signals:
                    ...
                    mock_adaptive.return_value = (
                        pd.Series(
                            [25.0, 25.0, 25.0, 25.0, 25.0], index=test_features.index
                        ),
                        pd.Series(
                            [75.0, 75.0, 75.0, 75.0, 75.0], index=test_features.index
                        ),
                    )
                    mock_make_signals.return_value = (
...
class TestRSISignalRoundtrip:
    ...
    def test_different_thresholds_produce_different_signals(self):
        ...
        try:
            ...
            count1 = generator.generate_for_ids(
                ids=[9997], signal_config=config1, full_refresh=True, dry_run=False
            )
            ...
            count2 = generator.generate_for_ids(
                ids=[9997], signal_config=config2, full_refresh=True, dry_run=False
            )
