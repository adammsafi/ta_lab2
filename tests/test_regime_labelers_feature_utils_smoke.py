...
def test_daily_labeler_with_minimal_features_smoke():
    ...
    df = pd.DataFrame({"close": price}, index=idx).reset_index().rename(columns={"index":"timestamp"})
    df = df.set_index("timestamp")

    df = ensure_regime_features(df, tf="D")
    ctx = assess_data_budget(daily=df)
    lab = label_layer_daily(df, mode=ctx.feature_tier)
    assert lab.iloc[-1] in {"Up-Low-Normal","Up-Normal-Normal","Up-High-Normal",
                            "Sideways-Low-Normal","Sideways-Normal-Normal","Down-Normal-Normal"}
