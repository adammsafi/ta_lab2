...
class TestTAFeatureBollingerBands(unittest.TestCase):
    ...
    def test_compute_bollinger(self):
        ...
        if valid_idx.any():
            self.assertTrue((df.loc[valid_idx, 'bb_up_20_2'] >= df.loc[valid_idx, 'bb_ma_20']).all())
            self.assertTrue((df.loc[valid_idx, 'bb_ma_20'] >= df.loc[valid_idx, 'bb_lo_20_2']).all())
...
class TestTAFeatureNullHandling(unittest.TestCase):
    ...
    def test_null_handling_interpolate(self):
        ...
        df_source = pd.DataFrame({
            ...
            'close': [100.0, 101.0, np.nan, np.nan, 104.0, 105.0, 106.0, 107.0, 108.0, 109.0],
        })
