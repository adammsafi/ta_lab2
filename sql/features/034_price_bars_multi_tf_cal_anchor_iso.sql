CREATE TABLE IF NOT EXISTS public.price_bars_multi_tf_cal_anchor_iso (
  id          integer                     NOT NULL,
  tf          text                        NOT NULL,
  ...
  CONSTRAINT pk_price_bars_multi_tf_cal_anchor_iso PRIMARY KEY (id, tf, bar_seq)
);

CREATE INDEX IF NOT EXISTS ix_price_bars_multi_tf_cal_anchor_iso_id_tf_timeclose
  ON public.price_bars_multi_tf_cal_anchor_iso (id, tf, time_close);

SELECT
	DISTINCT(tf),
	tf_days
FROM price_bars_multi_tf_cal_anchor_iso
ORDER BY tf_days, tf
