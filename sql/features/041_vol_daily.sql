...
CREATE TABLE IF NOT EXISTS public.vol_daily (
    -- Primary key
    id              INTEGER NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_vol_daily_id_ts
ON public.vol_daily (id, ts DESC);

-- Table comment
COMMENT ON TABLE public.vol_daily IS
'Daily volatility measures calculated from price_bars_1d OHLC. All volatility measures annualized using sqrt(252) for trading days. Includes Parkinson (range-based), Garman-Klass (OHLC-based), Rogers-Satchell (drift-independent), ATR, and rolling historical volatility from log returns.';

-- Column comments
COMMENT ON COLUMN public.vol_daily.vol_parkinson_20 IS 'Parkinson (1980) range-based volatility, 20-day window, annualized';
COMMENT ON COLUMN public.vol_daily.vol_gk_20 IS 'Garman-Klass (1980) OHLC volatility, 20-day window, annualized';
COMMENT ON COLUMN public.vol_daily.vol_rs_20 IS 'Rogers-Satchell (1991) drift-independent volatility, 20-day window, annualized';
COMMENT ON COLUMN public.vol_daily.atr_14 IS 'Average True Range (Wilder), 14-day period';
COMMENT ON COLUMN public.vol_daily.vol_log_roll_20 IS 'Rolling standard deviation of log returns, 20-day window, annualized';
