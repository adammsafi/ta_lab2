...
CREATE INDEX IF NOT EXISTS idx_cycle_stats_id_tf_as_ts
ON public.cycle_stats (id, tf, alignment_source, ts DESC);

-- Index for finding ATH events
CREATE INDEX IF NOT EXISTS idx_cycle_stats_ath_events
ON public.cycle_stats (id, tf, is_at_ath)
WHERE is_at_ath = TRUE;

COMMENT ON TABLE public.cycle_stats IS
