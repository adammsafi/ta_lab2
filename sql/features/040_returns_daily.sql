-- sql/features/040_returns_daily.sql
...
--
-- Source: price_bars_1d (daily validated bars)
-- State tracking: feature_state (feature_type='returns')
...
CREATE INDEX IF NOT EXISTS idx_returns_daily_id_ts
ON public.returns_daily (id, ts DESC);

-- Comment
COMMENT ON TABLE public.returns_daily IS
'Daily returns calculated from price_bars_1d. Lookback windows derived from dim_timeframe tf_days.';
