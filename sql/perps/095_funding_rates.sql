-- 095_funding_rates.sql
-- Reference DDL for funding_rates table.
...
CREATE INDEX idx_funding_rates_symbol_ts
    ON public.funding_rates (symbol, ts DESC);

CREATE INDEX idx_funding_rates_venue_symbol_ts
    ON public.funding_rates (venue, symbol, ts DESC);
...
COMMENT ON COLUMN public.funding_rates.venue IS
    'Exchange venue: binance, hyperliquid, bybit, dydx, aevo, aster, or lighter.';

COMMENT ON COLUMN public.funding_rates.symbol IS
    'Base asset symbol without quote currency, e.g. BTC, ETH.';

COMMENT ON COLUMN public.funding_rates.ts IS
    'UTC settlement timestamp for this funding rate payment.';

COMMENT ON COLUMN public.funding_rates.tf IS
...
COMMENT ON COLUMN public.funding_rates.raw_tf IS
