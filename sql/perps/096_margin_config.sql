-- 096_margin_config.sql
-- Reference DDL for margin_config dimension table.
...
COMMENT ON COLUMN public.margin_config.notional_floor IS
    'Minimum position notional (in USDT) for this tier to apply. Tier 1 is always 0.';

COMMENT ON COLUMN public.margin_config.notional_cap IS
    'Maximum position notional for this tier. NULL for the highest (last) tier.';

COMMENT ON COLUMN public.margin_config.initial_margin_rate IS
    'Initial margin rate as decimal fraction (e.g. 0.008 = 0.8%).';

COMMENT ON COLUMN public.margin_config.maintenance_margin_rate IS
...
INSERT INTO public.margin_config
