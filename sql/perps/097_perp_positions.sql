-- 097_perp_positions.sql
-- Reference DDL for perp_positions table.
-- Phase 51: Perps Readiness.
-- ASCII-only: no box-drawing or non-ASCII characters (Windows cp1252 compatibility).
--
-- Purpose: Track perpetual futures positions with margin state, separate from positions (spot).
-- Kept separate from positions to avoid touching the existing spot position CHECK constraint
-- (positions has CHECK (exchange IN ('coinbase','kraken','paper','aggregate')) which
...
COMMENT ON TABLE public.perp_positions IS
    'Perpetual futures position tracking with margin state. '
    'PK: (venue, symbol, strategy_id). '
    'Kept separate from positions (spot) because positions has a '
...
COMMENT ON COLUMN public.perp_positions.side IS
    'Position direction: long, short, or flat (closed).';

COMMENT ON COLUMN public.perp_positions.quantity IS
    'Position size in base asset units. 0 when side = flat.';

COMMENT ON COLUMN public.perp_positions.avg_entry_price IS
    'Volume-weighted average entry price in USDT.';

COMMENT ON COLUMN public.perp_positions.mark_price IS
    'Current mark price used for unrealized P&L and margin calculations.';

COMMENT ON COLUMN public.perp_positions.unrealized_pnl IS
    'Unrealized profit or loss in USDT at current mark_price.';

COMMENT ON COLUMN public.perp_positions.margin_mode IS
    'isolated: margin dedicated to this position only. '
    'cross: shared wallet margin across all positions at venue.';

COMMENT ON COLUMN public.perp_positions.leverage IS
    'Leverage multiplier for this position (1x to 125x, venue-dependent).';

COMMENT ON COLUMN public.perp_positions.allocated_margin IS
    'Collateral in USDT currently allocated to this position.';

COMMENT ON COLUMN public.perp_positions.maintenance_margin IS
    'Minimum margin in USDT required to keep position open (from margin_config tier).';

COMMENT ON COLUMN public.perp_positions.margin_utilization IS
...
COMMENT ON COLUMN public.perp_positions.liquidation_price IS
