TRUNCATE TABLE ema_multi_tf_u;

ALTER TABLE ema_multi_tf_u
    DROP CONSTRAINT IF EXISTS ema_multi_tf_u_pkey;
...
SELECT
    ...
    d1_roll_bar,
    d2_roll_bar
FROM ema_multi_tf_cal_anchor
ON CONFLICT (id, ts, tf, period, alignment_source) DO NOTHING;


-- 3) tf_day (legacy ema_multi_tf)
INSERT INTO ema_multi_tf_u (
    id,
    ts,
    tf,
...
SELECT
    ...
    NULL::double precision AS d1_roll_bar,
    NULL::double precision AS d2_roll_bar
FROM ema_multi_tf_v2
ON CONFLICT (id, ts, tf, period, alignment_source) DO NOTHING;

SELECT 'cal' AS src, COUNT(*) FROM ema_multi_tf_cal
UNION ALL
SELECT 'anchor', COUNT(*) FROM ema_multi_tf_cal_anchor
UNION ALL
SELECT 'tf_day', COUNT(*) FROM ema_multi_tf
UNION ALL
SELECT 'v2_daily', COUNT(*) FROM ema_multi_tf_v2
UNION ALL
SELECT 'u_total', COUNT(*) FROM ema_multi_tf_u;
