...
-- Reference DDL for cmc_drift_metrics table.
...
CREATE TABLE IF NOT EXISTS public.cmc_drift_metrics (
    ...
    -- -------------------------------------------------------------------------
    pit_replay_run_id   UUID            NULL,   -- FK concept: cmc_backtest_runs (PIT replay)
    cur_replay_run_id   UUID            NULL,   -- FK concept: cmc_backtest_runs (current-data replay)
