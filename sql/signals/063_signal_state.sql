-- signal_state: Position state tracking for stateful signals
--
...
CREATE INDEX IF NOT EXISTS idx_signal_state_open_positions
    ON public.signal_state (signal_type, signal_id)
    WHERE open_position_count > 0;

COMMENT ON TABLE public.signal_state IS
'Position state tracking for signal generation. Enables incremental refresh and position lifecycle management.';

COMMENT ON COLUMN public.signal_state.last_entry_ts IS
'Timestamp of most recent signal entry for this asset/signal combination';

COMMENT ON COLUMN public.signal_state.last_exit_ts IS
'Timestamp of most recent position exit for this asset/signal combination';

COMMENT ON COLUMN public.signal_state.open_position_count IS
