-- signals_rsi_mean_revert: RSI mean reversion trading signals
--
...
CREATE TABLE IF NOT EXISTS public.signals_rsi_mean_revert (
    -- Primary key
    id                      INTEGER         NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_cmc_signals_rsi_mr_open_positions
    ON public.signals_rsi_mean_revert (id, signal_id, position_state)
...
CREATE INDEX IF NOT EXISTS idx_cmc_signals_rsi_mr_backtest
    ON public.signals_rsi_mean_revert (signal_id, ts);

-- Index for PnL analysis
CREATE INDEX IF NOT EXISTS idx_cmc_signals_rsi_mr_closed
    ON public.signals_rsi_mean_revert (signal_id, position_state)
...
CREATE INDEX IF NOT EXISTS idx_cmc_signals_rsi_mr_extreme
    ON public.signals_rsi_mean_revert (rsi_at_entry)
    WHERE position_state = 'closed' AND rsi_at_entry IS NOT NULL;

COMMENT ON TABLE public.signals_rsi_mean_revert IS
...
COMMENT ON COLUMN public.signals_rsi_mean_revert.rsi_at_entry IS
'RSI value at entry signal. Used to analyze relationship between entry RSI and PnL.';

COMMENT ON COLUMN public.signals_rsi_mean_revert.rsi_at_exit IS
