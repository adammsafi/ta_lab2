-- signals_ema_crossover: EMA crossover trading signals
--
...
CREATE TABLE IF NOT EXISTS public.signals_ema_crossover (
    -- Primary key
    id                      INTEGER         NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_cmc_signals_ema_xo_open_positions
    ON public.signals_ema_crossover (id, signal_id, position_state)
...
CREATE INDEX IF NOT EXISTS idx_cmc_signals_ema_xo_backtest
    ON public.signals_ema_crossover (signal_id, ts);

-- Index for PnL analysis
CREATE INDEX IF NOT EXISTS idx_cmc_signals_ema_xo_closed
    ON public.signals_ema_crossover (signal_id, position_state)
    WHERE position_state = 'closed';

COMMENT ON TABLE public.signals_ema_crossover IS
...
COMMENT ON COLUMN public.signals_ema_crossover.feature_snapshot IS
'JSON snapshot of features at signal generation: {close, ema_fast, ema_slow, rsi, vol, ...}';

COMMENT ON COLUMN public.signals_ema_crossover.feature_version_hash IS
