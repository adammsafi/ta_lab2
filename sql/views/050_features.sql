...
-- Source tables:
--   - price_bars_multi_tf (OHLCV, all TFs)
--   - returns_bars_multi_tf (bar returns, canonical + roll columns)
--   - vol (volatility estimators)
--   - ta (technical indicators)
--
-- NOT included (different granularity — has period dimension):
--   - ema_multi_tf_u (EMA values) — query directly
--   - returns_ema_multi_tf_u (EMA returns) — query directly
--
-- Refresh pattern: Incremental via FeaturesStore
-- State tracking: feature_state (feature_type='daily_features')
...
CREATE TABLE IF NOT EXISTS public.features (
    ...
    -- ═══════════════════════════════════════════════════════════════
    -- OHLCV (from price_bars_multi_tf)
    -- ═══════════════════════════════════════════════════════════════
    ...
    -- ═══════════════════════════════════════════════════════════════
    -- Bar Returns (from returns_bars_multi_tf, join on roll=FALSE)
...
'Unified multi-TF bar-level feature store for ML pipelines. Materialized from prices, returns, vol, TA. EMAs excluded (different granularity — query ema_multi_tf_u directly).';
