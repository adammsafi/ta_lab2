-- cmc_ml_experiments: ML experiment run tracking
--
...
-- Extends the cmc_backtest_runs pattern to cover ML model training runs.
...
CREATE INDEX IF NOT EXISTS idx_ml_experiments_model_type
    ON public.cmc_ml_experiments(model_type);

CREATE INDEX IF NOT EXISTS idx_ml_experiments_created_at
    ON public.cmc_ml_experiments(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_ml_experiments_asset_ids
    ON public.cmc_ml_experiments USING GIN (asset_ids);

CREATE INDEX IF NOT EXISTS idx_ml_experiments_feature_set_hash
    ON public.cmc_ml_experiments(feature_set_hash);
...
COMMENT ON COLUMN public.cmc_ml_experiments.oos_precision IS
'Mean OOS precision (positive predictive value) across CV folds.';

COMMENT ON COLUMN public.cmc_ml_experiments.oos_recall IS
'Mean OOS recall (sensitivity) across CV folds.';

COMMENT ON COLUMN public.cmc_ml_experiments.oos_f1 IS
...
COMMENT ON COLUMN public.cmc_ml_experiments.optuna_n_trials IS
'Number of Optuna trials completed. NULL if Optuna not used.';

COMMENT ON COLUMN public.cmc_ml_experiments.optuna_best_params IS
