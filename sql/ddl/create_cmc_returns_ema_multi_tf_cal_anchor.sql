-- create_cmc_returns_ema_multi_tf_cal_anchor_unified.sql
--
...
ON public.cmc_returns_ema_multi_tf_cal_anchor_us (id, tf, period, series, roll, ts);
...
ON public.cmc_returns_ema_multi_tf_cal_anchor_iso (id, tf, period, series, roll, ts);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'ck_ret_ema_cal_anchor_us_series'
  ) THEN
    ALTER TABLE public.cmc_returns_ema_multi_tf_cal_anchor_us
      ADD CONSTRAINT ck_ret_ema_cal_anchor_us_series
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'ck_ret_ema_cal_anchor_us_state_series'
  ) THEN
    ALTER TABLE public.cmc_returns_ema_multi_tf_cal_anchor_us_state
      ADD CONSTRAINT ck_ret_ema_cal_anchor_us_state_series
      CHECK (series IN ('ema','ema_bar'));
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'ck_ret_ema_cal_anchor_iso_series'
  ) THEN
    ALTER TABLE public.cmc_returns_ema_multi_tf_cal_anchor_iso
      ADD CONSTRAINT ck_ret_ema_cal_anchor_iso_series
      CHECK (series IN ('ema','ema_bar'));
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'ck_ret_ema_cal_anchor_iso_state_series'
  ) THEN
    ALTER TABLE public.cmc_returns_ema_multi_tf_cal_anchor_iso_state
      ADD CONSTRAINT ck_ret_ema_cal_anchor_iso_state_series
      CHECK (series IN ('ema','ema_bar'));
  END IF;
END$$;
