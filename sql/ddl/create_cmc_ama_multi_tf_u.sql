-- create_cmc_ama_multi_tf_u.sql
-- Creates:
--   public.cmc_ama_multi_tf_u       (unified AMA value table)
--   public.cmc_ama_multi_tf_u_state
--
...
CREATE TABLE IF NOT EXISTS public.cmc_ama_multi_tf_u_state (
    id               integer     NOT NULL,
    tf               text        NOT NULL,
    indicator        text        NOT NULL,
    params_hash      text        NOT NULL,
...
CREATE INDEX IF NOT EXISTS ix_cmc_ama_multi_tf_u_indicator
ON public.cmc_ama_multi_tf_u (indicator, params_hash, tf, ts);

-- Filter by alignment source for sync deduplication
CREATE INDEX IF NOT EXISTS ix_cmc_ama_multi_tf_u_alignment_source
ON public.cmc_ama_multi_tf_u (alignment_source, id, tf, ts);

-- Fast roll=TRUE filter
CREATE INDEX IF NOT EXISTS ix_cmc_ama_multi_tf_u_roll_true
ON public.cmc_ama_multi_tf_u (id, tf, indicator, params_hash, ts)
WHERE roll = TRUE;
