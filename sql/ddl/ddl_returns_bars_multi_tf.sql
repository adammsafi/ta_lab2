...
-- Source bars: public.price_bars_multi_tf
-- PK: (id, "timestamp", tf)
-- ============================

DROP TABLE IF EXISTS public.returns_bars_multi_tf CASCADE;

CREATE TABLE public.returns_bars_multi_tf (
    id                      integer       NOT NULL,
    ...
    CONSTRAINT returns_bars_multi_tf_pk PRIMARY KEY (id, "timestamp", tf)
);

CREATE INDEX IF NOT EXISTS ix_returns_bars_multi_tf_id_tf_ts
ON public.returns_bars_multi_tf (id, tf, "timestamp");

-- State table: watermark per (id, tf)
DROP TABLE IF EXISTS public.returns_bars_multi_tf_state CASCADE;

CREATE TABLE public.returns_bars_multi_tf_state (
    id              integer       NOT NULL,
    tf              text          NOT NULL,
    last_timestamp  timestamptz,
    updated_at      timestamptz   NOT NULL DEFAULT now(),
    CONSTRAINT returns_bars_multi_tf_state_pk PRIMARY KEY (id, tf)
);
