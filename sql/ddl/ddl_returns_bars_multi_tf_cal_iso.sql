...
-- Source bars: public.price_bars_multi_tf_cal_iso
-- PK: (id, "timestamp", tf)
-- ============================

DROP TABLE IF EXISTS public.returns_bars_multi_tf_cal_iso CASCADE;

CREATE TABLE public.returns_bars_multi_tf_cal_iso (
    id                      integer       NOT NULL,
    ...
    CONSTRAINT returns_bars_multi_tf_cal_iso_pk PRIMARY KEY (id, "timestamp", tf)
);

CREATE INDEX IF NOT EXISTS ix_returns_bars_multi_tf_cal_iso_id_tf_ts
ON public.returns_bars_multi_tf_cal_iso (id, tf, "timestamp");

DROP TABLE IF EXISTS public.returns_bars_multi_tf_cal_iso_state CASCADE;

CREATE TABLE public.returns_bars_multi_tf_cal_iso_state (
    id              integer       NOT NULL,
    tf              text          NOT NULL,
    last_timestamp  timestamptz,
    updated_at      timestamptz   NOT NULL DEFAULT now(),
    CONSTRAINT returns_bars_multi_tf_cal_iso_state_pk PRIMARY KEY (id, tf)
);
