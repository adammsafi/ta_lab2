-- =============================================================================
-- File: sql/ddl/price_bars__price_bars_multi_tf.sql
-- Purpose:
--   - Define price_bars_multi_tf (tf-day bars)
...
CREATE TABLE IF NOT EXISTS public.price_bars_multi_tf (
    id            INTEGER NOT NULL,
    ...
    CONSTRAINT pk_price_bars_multi_tf
        PRIMARY KEY (id, tf, bar_seq),

    CONSTRAINT uq_price_bars_multi_tf_timeclose
        UNIQUE (id, tf, time_close),

    CONSTRAINT ck_price_bars_multi_tf_tf_days
        CHECK (tf_days > 0),

    CONSTRAINT ck_price_bars_multi_tf_time_order
        CHECK (time_open < time_close),

    CONSTRAINT ck_price_bars_multi_tf_ohlc
        CHECK (high >= GREATEST(open, close) AND low <= LEAST(open, close) AND high >= low)
);

-- Helpful indexes for typical queries
CREATE INDEX IF NOT EXISTS ix_price_bars_multi_tf_id_tf_timeclose
    ON public.price_bars_multi_tf (id, tf, time_close);

CREATE INDEX IF NOT EXISTS ix_price_bars_multi_tf_tf_timeclose
    ON public.price_bars_multi_tf (tf, time_close);

CREATE INDEX IF NOT EXISTS ix_price_bars_multi_tf_id_tf_barseq
    ON public.price_bars_multi_tf (id, tf, bar_seq);
...
WHERE table_schema='public'
  AND table_name='price_bars_multi_tf'
...
WHERE tc.table_schema='public'
  AND tc.table_name='price_bars_multi_tf'
...
SELECT *
FROM public.price_bars_multi_tf b, last_full lf
