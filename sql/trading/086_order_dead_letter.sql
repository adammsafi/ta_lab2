-- order_dead_letter: Dead-letter queue for failed OMS operations.
...
CREATE TABLE public.order_dead_letter (
    ...
    CONSTRAINT pk_order_dead_letter PRIMARY KEY (dlq_id),
...
CREATE INDEX idx_dlq_status_retry
    ON public.order_dead_letter (status, retry_after)
    WHERE status = 'pending';

-- Partial index: look up DLQ entries by related order
CREATE INDEX idx_dlq_order_id
    ON public.order_dead_letter (order_id)
    WHERE order_id IS NOT NULL;

-- Time-ordered access for recent failures
CREATE INDEX idx_dlq_created_at
    ON public.order_dead_letter (created_at DESC);

COMMENT ON TABLE public.order_dead_letter IS
