-- regime_flips: Regime transition events per asset/timeframe/layer
--
...
CREATE TABLE IF NOT EXISTS public.regime_flips (
    -- Primary key
    id                  INTEGER         NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_regime_flips_id_tf_layer
    ON public.regime_flips (id, tf, layer);

COMMENT ON TABLE public.regime_flips IS
...
COMMENT ON COLUMN public.regime_flips.layer IS
'Which regime layer flipped: L0 (monthly), L1 (weekly), L2 (daily), or composite (resolved policy).';

COMMENT ON COLUMN public.regime_flips.duration_bars IS
