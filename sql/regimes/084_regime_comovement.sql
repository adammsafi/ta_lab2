-- regime_comovement: Pairwise EMA comovement statistics
--
...
CREATE TABLE IF NOT EXISTS public.regime_comovement (
    -- Primary key
    id                  INTEGER         NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_regime_comovement_id_tf
    ON public.regime_comovement (id, tf);

COMMENT ON TABLE public.regime_comovement IS
...
COMMENT ON COLUMN public.regime_comovement.ema_a IS
'First EMA series identifier, e.g. ''close_ema_20''. Matches column names in ema_multi_tf_u.';

COMMENT ON COLUMN public.regime_comovement.ema_b IS
'Second EMA series identifier, e.g. ''close_ema_50''. Matches column names in ema_multi_tf_u.';

COMMENT ON COLUMN public.regime_comovement.sign_agree_rate IS
'Fraction of bars (0.0-1.0) where the daily delta of EMA_a and EMA_b have the same sign. 1.0 = always aligned, 0.5 = random.';

COMMENT ON COLUMN public.regime_comovement.best_lead_lag IS
'Lag in bars that maximises the cross-correlation. Positive means EMA_b leads EMA_a (EMA_b changes first).';

COMMENT ON COLUMN public.regime_comovement.computed_at IS
