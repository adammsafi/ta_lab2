-- regimes: Market regime labels and resolved policy per asset/timeframe
--
...
CREATE INDEX IF NOT EXISTS idx_regimes_id_tf_key
    ON public.regimes (id, tf, regime_key);

-- Index for global regime stats queries (e.g. how common is each regime)
CREATE INDEX IF NOT EXISTS idx_regimes_key
    ON public.regimes (regime_key);

-- Index for latest-regime queries (most recent bar per asset/TF)
CREATE INDEX IF NOT EXISTS idx_regimes_id_tf_ts_desc
    ON public.regimes (id, tf, ts DESC);

COMMENT ON TABLE public.regimes IS
