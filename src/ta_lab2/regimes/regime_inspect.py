...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Inspect multi-TF regime and resolved policy"
    )
    ...
    L0 = (
        label_layer_monthly(df_m, mode=mode).iloc[-1]
        if (df_m is not None and ctx.enabled_layers["L0"])
        else None
    )
    L1 = (
        label_layer_weekly(df_w, mode=mode).iloc[-1]
        if (df_w is not None and ctx.enabled_layers["L1"])
        else None
    )
    L2 = (
        label_layer_daily(df_d, mode=mode).iloc[-1]
        if (df_d is not None and ctx.enabled_layers["L2"])
        else None
    )
    L3 = (
        label_layer_intraday(df_i).iloc[-1]
        if (df_i is not None and ctx.enabled_layers["L3"])
        else None
    )
