...
def sign_from_series(
    df: pd.DataFrame, src_col: str, out_col: str | None = None
) -> pd.DataFrame:
    """
...
def detect_flips(
    df: pd.DataFrame, sign_col: str, min_separation: int = 1
) -> pd.DataFrame:
    """
...
def label_regimes_from_flips(
    n_rows: int, flip_idx: List[int], start_regime: int = 0
) -> np.ndarray:
    """
...
def attach_regimes(
    df: pd.DataFrame, regime_ids: np.ndarray, col: str = "regime_id"
) -> pd.DataFrame:
    """
...
def regime_stats(
    df: pd.DataFrame, regime_col: str = "regime_id", ret_col: str = "close_ret_1"
) -> pd.DataFrame:
    """
