...
if __name__ == "__main__":
    ...
    if dt.isna().all():
        raise KeyError(
            "Could not parse any timestamps from 'timeOpen' or 'timestamp' columns."
        )
    ...
    weekly = bin_by_calendar(df2, "timeopen", "W-SUN").rename(
        columns={"period_end": "date"}
    )
    weekly["timeframe"] = "1W"

    monthly = bin_by_calendar(df2, "timeopen", "MS").rename(
        columns={"period_end": "date"}
    )
    monthly["timeframe"] = "1M"

    # Daily from raw rows
    daily = df2.rename(columns={"timeopen": "date"})[
    ...
    def enrich(bars: pd.DataFrame) -> pd.DataFrame:
        add_returns(
            bars, close_col="close"
        )  # adds close_ret_1 by default (per your local impl)
    ...
    w = (
        weekly_en[["date", "close_ema_21_d1"]]
        .sort_values("date")
        .rename(columns={"close_ema_21_d1": "close_ema_21_d1_w"})
    )
    cmp = pd.merge_asof(d, w, on="date", direction="backward")
    cmp["ema21_d1_agree"] = (cmp["close_ema_21_d1"] * cmp["close_ema_21_d1_w"]) > 0
    print(
        "Daily vs Weekly EMA21 slope agreement:", f"{cmp['ema21_d1_agree'].mean():.2%}"
    )
    ...
    def check_boundary(dt_str):
        ...
        if not drow.empty and not wrow.empty:
            print(
                ...
                dt_str,
                "daily open:",
                float(_scal(drow["open"])),
                "weekly open:",
                float(_scal(wrow["open"])),
            )
    ...
    daily_sig = sign_from_series(
        daily_en.copy(), "close_ema_21_d1", out_col="d_slope_sign"
    )
    ...
    if "close_ret_1" not in daily_reg.columns:
        ...
        candidates = [
            c for c in daily_reg.columns if c.startswith("close") and "ret" in c
        ]
        if candidates:
            daily_reg["close_ret_1"] = daily_reg[candidates[0]]
        else:
            # Compute forward simple return: (close[t+1] / close[t]) - 1
            daily_reg["close_ret_1"] = (
                daily_reg["close"].shift(-1) / daily_reg["close"]
            ) - 1
