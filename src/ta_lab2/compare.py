import pandas as pd
from .resample import bin_by_calendar
...
def build_timeframe(symbol: str, base_daily: pd.DataFrame, tf: str, dt_col="timeopen"):
    if tf == "1D":
        ...
        bars = bin_by_calendar(base_daily, dt_col, "W-SUN").rename(
            columns={"period_end": "date"}
        )
        bars["timeframe"] = "1W"
    elif tf == "1M":
        bars = bin_by_calendar(base_daily, dt_col, "MS").rename(
            columns={"period_end": "date"}
        )
        bars["timeframe"] = "1M"
    else:
    ...
    bars["symbol"] = symbol
    prices = bars[
        ["date", "symbol", "timeframe", "open", "high", "low", "close", "volume"]
    ].copy()

    cal = prices[["date"]].drop_duplicates().copy()
    expand_datetime_features_inplace(
        cal, "date", prefix="date"
    )  # adds seasons/moon (if available)
    cal["symbol"] = symbol
    cal["timeframe"] = prices["timeframe"].iloc[0]
