...
def _apply_ohlcv_agg(w: pd.DataFrame,
    ...
    agg = {}
    o,h,l,c = ohlc_cols
    if o in w.columns: agg[o] = "first"
    if h in w.columns: agg[h] = "max"
    if l in w.columns: agg[l] = "min"
    if c in w.columns: agg[c] = "last"
    for sc in sum_cols:
        if sc in w.columns:
            agg[sc] = "sum"
    if extra_aggs:
...
def _flatten_agg_columns(r: pd.DataFrame) -> pd.DataFrame:
    ...
    if isinstance(r.columns, pd.MultiIndex):
        r.columns = ["_".join([c for c in tup if c]) for tup in r.columns.to_flat_index()]
    return r
...
def bin_by_calendar(df: pd.DataFrame,
                    dt_col: str,
                    freq: str,
                    *,
                    ohlc_cols=("open","high","low","close"),
                    ...
                    closed: Optional[str] = None) -> pd.DataFrame:
    ...
    agg = _apply_ohlcv_agg(w, ohlc_cols=ohlc_cols, sum_cols=sum_cols, extra_aggs=extra_aggs)

    r = (w.resample(freq, label=label, closed=closed)
           .agg(agg)
           .dropna(how="all")
           .reset_index()
...
def bin_by_season(df: pd.DataFrame,
                  dt_col: str,
                  *,
                  ...
                  ohlc_cols=("open","high","low","close"),
                  sum_cols=("volume",),
                  extra_aggs: Optional[Dict[str, str]] = None) -> pd.DataFrame:
    ...
    if season_col_exact and season_col_exact in df.columns:
        ...
        raise KeyError("Season column not found. Provide season_col_exact or season_col_approx.")
    ...
    for ts, lab in zip(w.index, w["_season_label"]):
        ...
        season_year.append(y); labels.append(lbl)
    ...
    ordinals = w["_season_year"].astype(int) * 4 + w["_season_lbl"].map(_SEASON_ORDER).astype(int)
    ...
    if n <= 1:
        r["season_bin"] = r["_season_year"].astype(int).astype(str) + "-" + r["_season_lbl"]
    else:
        name_map = (w.groupby("_season_block")
                     .apply(lambda g2: f"{int(g2['_season_year'].iloc[0])}-{g2['_season_lbl'].iloc[0]}__to__{int(g2['_season_year'].iloc[-1])}-{g2['_season_lbl'].iloc[-1]}")
                     .rename("season_bin")).to_dict()
    ...
    rest = [c for c in r.columns if c not in drop_these and c not in keep_first and c not in grp_keys]
