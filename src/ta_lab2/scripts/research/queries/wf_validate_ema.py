...
def load_df(path: str) -> pd.DataFrame:
    ...
    ts = next(
        ...
        if c in ("timestamp", "timeopen", "time open", "date", "timeclose", "time close")
    )
    # pick a close column
    px = "close" if "close" in d.columns else next(
        c for c in d.columns if "close" in c or c in ("price", "last")
    )
...
def main():
    ...
    ref = ref.sort_values(["mar", "sharpe", "cagr"], ascending=[False, False, False]).head(1)
    ...
    pairs = rolling_train_test_splits(START, END, train_days=730, test_days=182, step_days=182)
    ...
    if not rows:
        raise RuntimeError("No walk-forward TEST rows produced; check date ranges and data length.")
    ...
    cols = [
        "split", "p_fast_ema", "p_slow_ema", "test_start", "test_end",
        "trades", "total_return", "cagr", "mdd", "mar", "sharpe"
    ]
    print(out[cols])

    # Quick OOS sanity stats
    means = out[["total_return", "cagr", "mdd", "mar", "sharpe"]].mean(numeric_only=True).to_dict()
    print("\nOOS means:", means)
