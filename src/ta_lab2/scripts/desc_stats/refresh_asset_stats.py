...
Source table: returns_bars_multi_tf (roll=FALSE, canonical bars only)
Target table: asset_stats (PK: id, ts, tf)
State table:  asset_stats_state (watermark per id/tf)

Usage:
    python -m ta_lab2.scripts.desc_stats.refresh_asset_stats --ids all --tf 1D
    python -m ta_lab2.scripts.desc_stats.refresh_asset_stats --ids 1,2 --tf 1D --full-rebuild
    python -m ta_lab2.scripts.desc_stats.refresh_asset_stats --ids all --workers 4

Spyder run example:
runfile(
  r"C:\Users\asafi\Downloads\ta_lab2\src\ta_lab2\scripts\desc_stats\refresh_asset_stats.py",
...
def _process_one(engine: Engine, db_url: str, task: WorkerTask) -> int:
    ...
    1. Read watermark from asset_stats_state.
    2. Load ret_arith from returns_bars_multi_tf.
    3. Compute stats.
    4. DELETE + INSERT into asset_stats.
    5. Update watermark.
    ...
    with engine.begin() as conn:
        ...
        insert_sql = text(
            f"INSERT INTO public.asset_stats ({col_list}) VALUES ({placeholders})"
        )
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Compute per-asset rolling descriptive statistics into asset_stats."
    )
    p.add_argument(
        "--ids",
        default="all",
