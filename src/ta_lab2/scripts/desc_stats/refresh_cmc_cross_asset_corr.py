#!/usr/bin/env python
"""
refresh_cmc_cross_asset_corr.py
...
Data source: cmc_returns_bars_multi_tf (ret_arith WHERE roll = FALSE)
Output table: cmc_cross_asset_corr (id_a, id_b, ts, tf, window, pearson_r, pearson_p,
              spearman_r, spearman_p, n_obs)
State table:  cmc_cross_asset_corr_state (watermark tracking per id_a, id_b, tf)
Materialized: cmc_corr_latest refreshed after all TF writes

Pair ordering: id_a < id_b always (matches CHECK constraint on cmc_cross_asset_corr).
...
Usage:
    python -m ta_lab2.scripts.desc_stats.refresh_cmc_cross_asset_corr --ids all --tf 1D
    python -m ta_lab2.scripts.desc_stats.refresh_cmc_cross_asset_corr --ids 1,52 --tf 1D --dry-run
    python -m ta_lab2.scripts.desc_stats.refresh_cmc_cross_asset_corr --full-rebuild --workers 4

Spyder run example:
runfile(
  r"C:\\Users\\asafi\\Downloads\\ta_lab2\\src\\ta_lab2\\scripts\\desc_stats\\refresh_cmc_cross_asset_corr.py",
...
MAT_VIEW = "public.cmc_corr_latest"
...
def _load_all_asset_ids(engine, ids_filter: Optional[List[int]]) -> List[int]:
    """Load all asset IDs from dim_assets (or filter to provided list)."""
    with engine.connect() as conn:
        if ids_filter is None:
            rows = conn.execute(
                text("SELECT id FROM public.dim_assets ORDER BY id")
            ).fetchall()
...
def _load_returns_wide(
    ...
    Note: cmc_returns_bars_multi_tf uses "timestamp" column (reserved word, must be quoted).
    """
...
def _refresh_materialized_view(db_url: str) -> None:
    """Refresh cmc_corr_latest materialized view (CONCURRENTLY if possible)."""
    engine = _get_engine(db_url)
    try:
        with engine.begin() as conn:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "per timeframe and write to cmc_cross_asset_corr."
        )
    )
    parser.add_argument(
        "--ids",
