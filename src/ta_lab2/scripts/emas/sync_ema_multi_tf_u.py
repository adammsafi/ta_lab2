...
- alignment_source = suffix after 'cmc_ema_' in the source table name.
  e.g. public.ema_multi_tf -> multi_tf
...
Spyder:
  runfile(
    r"C:\\Users\\asafi\\Downloads\\ta_lab2\\src\\ta_lab2\\scripts\\emas\\sync_ema_multi_tf_u.py",
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Sync EMA source tables into public.ema_multi_tf_u (incremental)."
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
