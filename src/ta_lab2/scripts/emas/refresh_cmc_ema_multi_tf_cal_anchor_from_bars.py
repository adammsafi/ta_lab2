"""
Refresh calendar-anchor EMA tables using BaseEMARefresher architecture.
...
Migrated from: refresh_cmc_ema_multi_tf_cal_anchor_from_bars.py
"""
...
def _process_id_worker(task: WorkerTask) -> int:
    ...
    Supports tf_subset for TF-level parallelism (passed via WorkerTask).
...
class CalAnchorEMARefresher(BaseEMARefresher):
    ...
    Uses:
    - cmc_price_bars_multi_tf_cal_anchor_us or cmc_price_bars_multi_tf_cal_anchor_iso
    ...
    def __init__(
        ...
        self.scheme = scheme
        self.bars_table = f"cmc_price_bars_multi_tf_cal_anchor_{scheme}"
    ...
    def get_timeframes(self) -> list[str]:
        """Load calendar-anchor timeframes from bars table."""
        return []  # Not needed for cal_anchor scripts
    ...
    def compute_emas_for_id(
        ...
        **extra_args,
        ...
        """Compute calendar-anchor EMAs for single ID."""
        n = write_multi_timeframe_ema_cal_anchor_to_db(
            self.engine,
            [id_],
            scheme=self.scheme,
    ...
    def create_argument_parser(cls) -> argparse.ArgumentParser:
        """Create argument parser with calendar-anchor specific arguments."""
        # Use base parser to get standardized arguments including validation
        p = cls.create_base_argument_parser(
            description="Refresh calendar-anchor EMAs (US/ISO schemes) - refactored.",
        )
        ...
        p.add_argument("--out-us", default="cmc_ema_multi_tf_cal_anchor_us")
        p.add_argument("--out-iso", default="cmc_ema_multi_tf_cal_anchor_iso")
    ...
    @classmethod
    def from_cli_args(cls, args: argparse.Namespace) -> "CalAnchorEMARefresher":
    ...
    def from_cli_args_for_scheme(
        ...
        temp_state_config = EMAStateConfig(
            ...
            bars_table=f"cmc_price_bars_multi_tf_cal_anchor_{scheme}",
            bars_schema="public",
    ...
    def main_for_schemes(cls, argv=None) -> None:
        ...
        for scheme in schemes_to_run:
            print(f"\n{'=' * 80}")
            print(f"Running calendar-anchor EMA refresh for scheme: {scheme.upper()}")
            print(f"{'=' * 80}\n")
...
if __name__ == "__main__":
    CalAnchorEMARefresher.main_for_schemes()
