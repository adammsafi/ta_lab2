...
def load_stats_csv(path: str) -> str:
    ...
    cols = ["stat_id", "table_name", "test_name", "asset_id", "tf", "period",
            "status", "actual", "expected", "extra", "checked_at"]
    df = df[cols]
...
"""
    ...
    resp = client.chat.completions.create(
        model="gpt-4.1-mini",
        messages=[
            {"role": "system", "content": "You are a highly technical data QA assistant helping with EMA and multi-timeframe stats."},
