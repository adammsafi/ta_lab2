"""
Cross-sectional normalization refresh for cmc_features.
...
Usage:
    python -m ta_lab2.scripts.features.refresh_cmc_cs_norms --tf 1D
    python -m ta_lab2.scripts.features.refresh_cmc_cs_norms --all-tfs
    python -m ta_lab2.scripts.features.refresh_cmc_cs_norms --tf 4H --dry-run
"""
...
def refresh_cs_norms(engine: Engine, tf: str = "1D") -> int:
    """Refresh cross-sectional normalization columns in cmc_features for one TF.
...
def _get_all_tfs(engine: Engine) -> list[str]:
    """Return list of distinct TF values present in cmc_features."""
    sql = "SELECT DISTINCT tf FROM public.cmc_features ORDER BY tf"
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Refresh cross-sectional z-scores and ranks for 3 pilot features in cmc_features. "
...
Examples:
  ...
  # Refresh CS norms for all timeframes in cmc_features
  python -m ta_lab2.scripts.features.refresh_cmc_cs_norms --all-tfs

  # Preview SQL without executing (dry-run)
  python -m ta_lab2.scripts.features.refresh_cmc_cs_norms --tf 1D --dry-run
...
""",
    ...
    tf_group.add_argument(
        "--all-tfs",
        action="store_true",
        help="Process all timeframes present in cmc_features",
    )
