"""
Incremental daily volatility refresh from price_bars_1d.
...
State tracking uses public.feature_state (feature_type='vol').

Usage:
    # Compute for specific IDs
    python -m ta_lab2.scripts.features.refresh_vol_daily --ids 1,52

    # Compute for all IDs
    python -m ta_lab2.scripts.features.refresh_vol_daily --all

    # Full refresh (recompute from start date)
    python -m ta_lab2.scripts.features.refresh_vol_daily --ids 1,52 --full-refresh

    # Dry run (no writes)
    python -m ta_lab2.scripts.features.refresh_vol_daily --ids 1,52 --dry-run
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Incremental daily volatility refresh from price_bars_1d",
...
def get_all_ids(engine) -> list[int]:
    """
    Query all distinct IDs from price_bars_1d.
