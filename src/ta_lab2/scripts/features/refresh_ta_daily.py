"""
Incremental daily TA indicators refresh from price_bars_1d.

Computes RSI, MACD, Stochastic, Bollinger Bands, ATR, ADX with multiple
parameter sets using TAFeature module.

Usage:
    python -m ta_lab2.scripts.features.refresh_ta_daily --ids 1,52
    python -m ta_lab2.scripts.features.refresh_ta_daily --all
    python -m ta_lab2.scripts.features.refresh_ta_daily --full-refresh
    python -m ta_lab2.scripts.features.refresh_ta_daily --indicators rsi_14,macd_12_26_9
    python -m ta_lab2.scripts.features.refresh_ta_daily --dry-run

State tracked in public.feature_state (feature_type='ta').
"""
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Refresh ta_daily from price_bars_1d",
    )
    ...
    id_group.add_argument(
        "--all",
        action="store_true",
        help="Process all IDs from price_bars_1d",
    )
