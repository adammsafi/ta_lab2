...
from ta_lab2.notifications.telegram import send_alert, is_configured as telegram_configured
...
class GapIssue(ValidationIssue):
    ...
    def __init__(self, table: str, id_: int, missing_dates: list[str], expected: int, actual: int):
        super().__init__(
            issue_type='gap',
            severity='warning',
...
class RowcountIssue(ValidationIssue):
    ...
    def __init__(self, table: str, id_: int, expected: int, actual: int, diff_pct: float):
        super().__init__(
            issue_type='rowcount',
            severity='warning',
...
class FeatureValidator:
    ...
    def check_outliers(
        ...
        for column in columns:
            ...
            with self.engine.connect() as conn:
                col_exists = conn.execute(col_exists_query, {'table': table, 'column': column}).scalar()
                if not col_exists:
                    logger.debug(f"Column {column} does not exist in {table} - skipping")
                    continue
    ...
    def check_rowcounts(
        ...
        with self.engine.connect() as conn:
            exists = conn.execute(table_exists_query, {'table': table}).scalar()
            if not exists:
                logger.warning(f"Table {table} does not exist - skipping rowcount check")
                return issues
