"""
Incremental daily returns refresh from price_bars_1d.

Computes daily returns with multiple lookback windows (1D, 3D, 5D, 7D, etc.)
using ReturnsFeature module.

Usage:
    python -m ta_lab2.scripts.features.refresh_returns_daily --ids 1,52
    python -m ta_lab2.scripts.features.refresh_returns_daily --all
    python -m ta_lab2.scripts.features.refresh_returns_daily --full-refresh
    python -m ta_lab2.scripts.features.refresh_returns_daily --dry-run

State tracked in public.feature_state (feature_type='returns').
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Refresh returns_daily from price_bars_1d",
    )
