...
"""
sync_cmc_returns_bars_multi_tf_u.py

Incrementally sync bar returns from 5 source tables into:
  public.cmc_returns_bars_multi_tf_u

alignment_source derived from table name:
  cmc_returns_bars_multi_tf          -> multi_tf
  cmc_returns_bars_multi_tf_cal_us   -> multi_tf_cal_us
  ...etc

Run:
  python -m ta_lab2.scripts.returns.sync_cmc_returns_bars_multi_tf_u
  python -m ta_lab2.scripts.returns.sync_cmc_returns_bars_multi_tf_u --dry-run
"""
...
U_TABLE = "public.cmc_returns_bars_multi_tf_u"
PK_COLS = ["id", "timestamp", "tf", "alignment_source"]
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Sync bar returns tables into public.cmc_returns_bars_multi_tf_u."
    )
