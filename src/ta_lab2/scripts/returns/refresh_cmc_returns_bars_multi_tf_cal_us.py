...
State:
  public.cmc_returns_bars_multi_tf_cal_us_state  (watermark per (id, tf): last_time_close)
...
Semantics:
  ...
  - History recomputed only with --full-refresh (drops outputs + state for the selected table).

Notes:
  - This is required for *_cal_* bar tables where bar_seq may not be unique / stable.

Run (Spyder):
runfile(
  r"C:\Users\asafi\Downloads\ta_lab2\src\ta_lab2\scripts\returns\refresh_cmc_returns_bars_multi_tf_cal_us.py",
...
DEFAULT_OUT_TABLE = "public.cmc_returns_bars_multi_tf_cal_us"
DEFAULT_STATE_TABLE = "public.cmc_returns_bars_multi_tf_cal_us_state"


def _print(msg: str) -> None:
    print(f"[ret_bars_cal_us] {msg}")
...
def ensure_tables(engine: Engine, out_table: str, state_table: str) -> None:
    with engine.begin() as cxn:
        ...
        cxn.execute(text(f"CREATE INDEX IF NOT EXISTS ix_{out_table.split('.')[-1]}_time_close ON {out_table} (id, tf, time_close);"))
        cxn.execute(
            text(
                f"""
