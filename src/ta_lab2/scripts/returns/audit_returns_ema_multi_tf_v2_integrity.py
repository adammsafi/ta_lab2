...
Key semantics:
  - Returns are computed per series key: (id, tf, period, roll), ordered by ts
  - Expected coverage: n_ret == n_ema - 1  (per key)  [assuming builder inserts only rows with prev_ema]
  - No duplicate (id, ts, tf, period, roll) in returns
  - Gap sanity:
      - baseline: gap_days >= 1
      - optional stronger: gap_days <= gap_mult * tf_days_nominal (if dim_timeframe provides tf_days_nominal)
  - Alignment:
...
Auto-dating:
  If --out is empty, auto-generates:
    audit_returns_ema_multi_tf_v2_YYYYMMDD

Run (Spyder):
runfile(
  r"C:\Users\asafi\Downloads\ta_lab2\src\ta_lab2\scripts\returns\audit_returns_ema_multi_tf_v2_integrity.py",
...
DEFAULT_RET_TABLE = "public.cmc_returns_ema_multi_tf_v2"
DEFAULT_DIM_TIMEFRAME = "public.dim_timeframe"


def _print(msg: str) -> None:
    print(f"[audit_ret_ema_v2] {msg}")
...
def _today_yyyymmdd() -> str:
    # Good enough for filenames; will reflect your local machine time.
    return datetime.now().strftime("%Y%m%d")


def _resolve_out_name(out_arg: str, default_prefix: str) -> str:
    """
    If --out is empty, auto-generate a dated base name:
      <default_prefix>_YYYYMMDD
    """
...
def main() -> None:
    ...
    default_prefix = "audit_returns_ema_multi_tf_v2"
    out_name = _resolve_out_name(args.out, default_prefix)
    out_dir = Path(args.out_dir).expanduser().resolve()
    out_base = out_dir / out_name  # always write outputs (auto-dated if not specified)
    ...
    # 1) Coverage: per (id, tf, period, roll) returns rows == ema rows - 1
    coverage_sql = f"""
    WITH e AS (
    ...
    # 3) Gap summary and anomalies (baseline + optional tf_days_nominal threshold)
    gap_sql = f"""
    SELECT
      id,
      tf,
    ...
    # Write CSVs (always; base name is auto-dated if --out omitted)
