...
What it checks:
  1) Coverage: returns rows == bars rows - 1 per (id, tf)
  2) Duplicates: no duplicate (id, tf, time_close) in returns
  3) gap_days sanity: summarizes gaps + flags gap_days NULL/<=0 rows
  4) Null policy: counts NULLs in prev_close / ret_arith / ret_log
  5) Alignment: every returns (id,tf,time_close) exists in bars

CSV outputs:
  Pass --out-dir and --out to write:
    <out>_coverage.csv
    <out>_coverage_bad.csv (only if diffs exist)
    <out>_dups.csv
...
Run (Spyder):
runfile(
  r"C:\Users\asafi\Downloads\ta_lab2\src\ta_lab2\scripts\returns\audit_returns_bars_multi_tf_cal_anchor_iso_integrity.py",
  wdir=r"C:\Users\asafi\Downloads\ta_lab2",
  args="--out-dir audits/returns --out audit_returns_bars_multi_tf_cal_anchor_iso"
)
"""
...
DEFAULT_RET_TABLE = "public.cmc_returns_bars_multi_tf_cal_anchor_iso"


def _print(msg: str) -> None:
    print(f"[audit_ret_bars_cal_anchor_iso] {msg}")
