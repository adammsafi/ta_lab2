#!/usr/bin/env python
"""
Refresh RSI mean reversion signals from features.
...
Usage:
    ...
    python refresh_signals_rsi_mean_revert.py

    # Full refresh for specific assets
    python refresh_signals_rsi_mean_revert.py --ids 1 52 --full-refresh

    # Use adaptive rolling percentile thresholds
    python refresh_signals_rsi_mean_revert.py --adaptive

    # Specific signal only
    python refresh_signals_rsi_mean_revert.py --signal-id 4

    # Dry run (validation without database writes)
    python refresh_signals_rsi_mean_revert.py --dry-run --verbose

Configuration:
    - Thresholds loaded from dim_signals params (default: lower=30, upper=70)
    - Signal types: 'rsi_mean_revert' in dim_signals
    - Output table: signals_rsi_mean_revert
    - State tracking: signal_state (per id, signal_type, signal_id)
...
Examples:
    ...
    python refresh_signals_rsi_mean_revert.py --full-refresh --adaptive -v
"""
...
def _get_all_asset_ids(engine) -> list[int]:
    """
    Query all unique asset IDs from features.
    ...
    """
    sql = text("SELECT DISTINCT id FROM public.features WHERE tf = '1D' ORDER BY id")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Refresh RSI mean reversion signals from features",
...
Examples:
  ...
  %(prog)s --dry-run -v
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        help="Asset IDs to process (default: all from features)",
    )
    parser.add_argument(
        "--signal-id",
        type=int,
    ...
    if args.ids:
        ...
        try:
            ...
            logger.info(f"Processing {len(ids)} assets from features")
        except Exception as e:
