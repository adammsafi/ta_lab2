#!/usr/bin/env python
"""
Refresh EMA crossover signals from features.
...
Usage:
    python -m ta_lab2.scripts.signals.refresh_signals_ema_crossover --ids 1,52
    python -m ta_lab2.scripts.signals.refresh_signals_ema_crossover --all
    python -m ta_lab2.scripts.signals.refresh_signals_ema_crossover --signal-id 1
    python -m ta_lab2.scripts.signals.refresh_signals_ema_crossover --full-refresh
    python -m ta_lab2.scripts.signals.refresh_signals_ema_crossover --dry-run

Signal configurations loaded from dim_signals table (signal_type='ema_crossover').
State tracked in public.signal_state.
"""
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Refresh EMA crossover signals from features",
    )
    ...
    id_group.add_argument(
        "--all",
        action="store_true",
        help="Process all IDs from features",
    )
...
def get_all_asset_ids(engine) -> list[int]:
    """
    Query all distinct asset IDs from features.
...
def load_ids(engine, ids_arg: Optional[str] = None, all_ids: bool = False) -> list[int]:
    ...
    Args:
        ...
        all_ids: If True, load all IDs from features
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.dry_run:
        ...
        logger.info("Output table: public.signals_ema_crossover")
        return 0
