"""
Refresh calendar-anchor AMA tables from bar data.

Targets:
  - public.cmc_ama_multi_tf_cal_anchor_us   (US calendar-anchor bars)
  - public.cmc_ama_multi_tf_cal_anchor_iso  (ISO calendar-anchor bars)

Uses:
  - CalAnchorUSAMAFeature  / CalAnchorISOAMAFeature from ama_multi_tf_cal_anchor
  - BaseAMARefresher for standardised CLI, state management, and parallelism

CLI usage:
  # Refresh US calendar anchor scheme for asset 1
  python -m ta_lab2.scripts.amas.refresh_cmc_ama_multi_tf_cal_anchor_from_bars \\
      --ids 1 --all-tfs --scheme us

  # Refresh ISO calendar anchor scheme for all assets
  python -m ta_lab2.scripts.amas.refresh_cmc_ama_multi_tf_cal_anchor_from_bars \\
      --ids all --all-tfs --scheme iso

  # Refresh both anchor schemes for assets 1 and 52
  python -m ta_lab2.scripts.amas.refresh_cmc_ama_multi_tf_cal_anchor_from_bars \\
      --ids 1,52 --all-tfs --scheme both

  # KAMA only, full rebuild for US anchor scheme
  python -m ta_lab2.scripts.amas.refresh_cmc_ama_multi_tf_cal_anchor_from_bars \\
...
from ta_lab2.features.ama.ama_multi_tf_cal_anchor import (
...
SCHEME_MAP = {
    "us": {
        ...
        "bars_table": "cmc_price_bars_multi_tf_cal_anchor_us",
        "output_table": "cmc_ama_multi_tf_cal_anchor_us",
        "state_table": "cmc_ama_multi_tf_cal_anchor_us_state",
        "description": "US calendar-anchor AMA",
    },
    "iso": {
        "feature_class": CalAnchorISOAMAFeature,
        "bars_table": "cmc_price_bars_multi_tf_cal_anchor_iso",
        "output_table": "cmc_ama_multi_tf_cal_anchor_iso",
        "state_table": "cmc_ama_multi_tf_cal_anchor_iso_state",
        "description": "ISO calendar-anchor AMA",
    },
}
...
def _cal_anchor_ama_worker(task: AMAWorkerTask) -> int:
    """
    Worker function for parallel calendar-anchor AMA computation.
    ...
    Uses CalAnchorUSAMAFeature or CalAnchorISOAMAFeature based on
    task.extra_config["scheme"].
    ...
    _logger = logging.getLogger(f"ama_cal_anchor_worker.{task.asset_id}")
    ...
    try:
        ...
        # Load TF specs for this calendar anchor scheme
...
class CalAnchorAMARefresher(BaseAMARefresher):
    """
    AMA refresher for calendar-anchor tables (US/ISO schemes).

    Handles --scheme us/iso/both by running one or two scheme-specific
    refresher passes. Each pass uses the corresponding CalAnchorUSAMAFeature
    or CalAnchorISOAMAFeature and its own state table.
    ...
    def __init__(self, scheme: str = "us") -> None:
        """
        Initialise CalAnchorAMARefresher.

        Args:
            scheme: Calendar anchor scheme for this refresher instance ("us" or "iso").
    ...
    def get_description(self) -> str:
        return (
            "Refresh calendar-anchor AMA tables (US/ISO schemes).\n\n"
            "Data sources:\n"
            "  cmc_price_bars_multi_tf_cal_anchor_us  -> cmc_ama_multi_tf_cal_anchor_us\n"
            "  cmc_price_bars_multi_tf_cal_anchor_iso -> cmc_ama_multi_tf_cal_anchor_iso\n\n"
    ...
    def create_argument_parser(cls) -> argparse.ArgumentParser:
        """Create CLI argument parser with calendar-anchor-specific --scheme argument."""
        # BaseAMARefresher.create_argument_parser() calls cls() — needs no-arg constructor
        p = super().create_argument_parser()

        # Calendar-anchor specific arguments
        p.add_argument(
            "--scheme",
            default="both",
            choices=["us", "iso", "both"],
            help=("Calendar anchor scheme to refresh: us, iso, or both. Default: both"),
        )
        p.add_argument(
            "--out-us",
            default="cmc_ama_multi_tf_cal_anchor_us",
            help=(
                "Output table name for US anchor scheme "
                "(default: cmc_ama_multi_tf_cal_anchor_us)."
            ),
        )
        p.add_argument(
            "--out-iso",
            default="cmc_ama_multi_tf_cal_anchor_iso",
            help=(
                "Output table name for ISO anchor scheme "
                "(default: cmc_ama_multi_tf_cal_anchor_iso)."
            ),
        )
    ...
    def main_for_schemes(cls, argv=None) -> None:
        ...
        for scheme in schemes_to_run:
            print(f"\n{'=' * 80}")
            print(f"Running calendar-anchor AMA refresh for scheme: {scheme.upper()}")
            print(f"{'=' * 80}\n")
    ...
    def _run_for_scheme(self, args: argparse.Namespace, scheme: str) -> None:
        """
        Execute AMA refresh for a single calendar anchor scheme.

        Mirrors BaseAMARefresher.run() but selects feature class, output table,
        and state table based on the scheme. Uses _cal_anchor_ama_worker for
        multiprocessing.

        Args:
            args: Parsed argparse.Namespace.
            scheme: Calendar anchor scheme ("us" or "iso").
        """
        ...
        run_logger = setup_logging(
            name=f"CalAnchorAMARefresher.{scheme.upper()}",
            level=log_level,
            log_file=log_file,
            quiet=quiet,
            debug=debug,
        ...
        run_logger.info(
            "Starting CalAnchorAMARefresher[%s]: output=%s.%s state=%s",
            scheme.upper(),
            output_schema,
            output_table,
            state_table,
        ...
        if tf_arg:
            ...
            tf_subset = None
            run_logger.info(
                "Using all calendar anchor TFs for scheme=%s", scheme.upper()
            )
        else:
            run_logger.warning(
        ...
        if getattr(args, "dry_run", False):
            tf_count = (
                len(tf_subset)
                if tf_subset
                else f"(all calendar anchor TFs for {scheme.upper()})"
            )
            run_logger.info(
        ...
        if num_processes == 1 or len(tasks) == 1:
            results = [_cal_anchor_ama_worker(t) for t in tasks]
        else:
            try:
                with Pool(processes=num_processes) as pool:
                    results = pool.map(_cal_anchor_ama_worker, tasks)
            except Exception as exc:
...
if __name__ == "__main__":
    CalAnchorAMARefresher.main_for_schemes()
