...
Incrementally sync AMA value records from 5 source AMA tables into:
  public.cmc_ama_multi_tf_u

Rules:
- alignment_source = suffix after 'cmc_ama_' in the source table name.
  e.g. public.cmc_ama_multi_tf -> multi_tf
       public.cmc_ama_multi_tf_cal_us -> multi_tf_cal_us
...
Run:
  ...
  python -m ta_lab2.scripts.amas.sync_cmc_ama_multi_tf_u --dry-run
  python -m ta_lab2.scripts.amas.sync_cmc_ama_multi_tf_u --only multi_tf,multi_tf_cal_us
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Sync AMA value source tables into public.cmc_ama_multi_tf_u (incremental)."
        )
    )
    ap.add_argument(
        "--db-url",
