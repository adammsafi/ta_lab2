...
# ruff: noqa: E402
"""
Calendar-aligned price bars builder for US weeks (Sunday-start):
...
US CALENDAR SEMANTICS
---------------------
- Weeks start on Sunday (US convention, weekday=6)
...
# =============================================================================
# CalendarUSBarBuilder - Inherits from BaseBarBuilder
# =============================================================================


class CalendarUSBarBuilder(BaseBarBuilder):
    """
    Calendar-aligned US bar builder - builds calendar bars with US week convention.

    US Semantics:
    - Weeks start on Sunday (weekday=6)
    ...
    STATE_TABLE = "public.cmc_price_bars_multi_tf_cal_us_state"
    OUTPUT_TABLE = "public.cmc_price_bars_multi_tf_cal_us"
    ...
    def __init__(
        ...
        """
        Initialize Calendar US bar builder.
    ...
    def build_bars_for_id(
        ...
        if self.from_1d:
            alignment, anchor_mode = BUILDER_ALIGNMENT_MAP["cal_us"]
    ...
    def _build_snapshots_full_history_polars(
        ...
        Builds calendar-aligned bars with US week convention (Sunday start).
    ...
    def create_argument_parser(cls) -> argparse.ArgumentParser:
        """
        Create argument parser with calendar US specific arguments.

        Returns:
            ArgumentParser with all arguments configured
        """
        parser = cls.create_base_argument_parser(
            description="Build calendar-aligned US price bars (Sunday-start weeks) into cmc_price_bars_multi_tf_cal_us.",
        ...
        # Calendar US specific arguments
        parser.add_argument(
            "--from-1d",
            action="store_true",
    ...
    @classmethod
    def from_cli_args(cls, args: argparse.Namespace) -> "CalendarUSBarBuilder":
        """
    ...
    def _load_cal_specs_from_dim(cls, db_url: str) -> list[CalSpec]:
        ...
        Filters for US calendar specs:
        - Weeks: tf LIKE '%_CAL_US' (US Sunday-start weeks)
        ...
        if not specs:
            raise RuntimeError(
                "No calendar TFs found in dim_timeframe matching US filters."
            )
...
def main(argv=None) -> None:
    """
    CLI entry point for calendar US bar builder.

    Usage:
        python refresh_cmc_price_bars_multi_tf_cal_us.py --ids all
        python refresh_cmc_price_bars_multi_tf_cal_us.py --ids 1 52 825 --full-rebuild
        python refresh_cmc_price_bars_multi_tf_cal_us.py --ids all --from-1d
    """
    parser = CalendarUSBarBuilder.create_argument_parser()
    args = parser.parse_args(argv)
    builder = CalendarUSBarBuilder.from_cli_args(args)
    builder.run()
