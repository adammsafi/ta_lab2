    ...
    try:
        ...
        for spec in specs:
            try:
                ...
                stats["errors"] += 1
                print(
                    f"[bars_anchor_us] ERROR id={id_} tf={spec.tf}: {type(e).__name__}: {e}"
                )
...
def refresh_incremental(
    *,
    ...
    print(
        f"[bars_anchor_us] Incremental: {len(ids)} IDs × {len(specs)} TFs = {total_combinations:,} combinations (tz={tz})"
    )
    ...
    progress = ProgressTracker(
        total=len(args_list), log_interval=5, prefix="[bars_anchor_us]"
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.from_1d:
        ...
        print(
            f"[bars_anchor_us] Deriving anchored calendar {alignment} bars from cmc_price_bars_1d (partial periods allowed)"
        )
        if args.validate_derivation:
            print(
                "[bars_anchor_us] Validation mode: will compare derived bars to direct computation"
            )
    ...
    if args.full_rebuild:
        ...
        print(
            f"[bars_anchor_us] Full rebuild: {len(ids)} IDs × {len(specs)} TFs = {total_combinations:,} combinations"
        )
                ...
                if args.validate_derivation:
                    ...
                    if bars_direct_all:
                        ...
                        if not is_consistent:
                            print(
                                f"[bars_anchor_us] Derivation discrepancies for id={id_}: {discrepancies}"
                            )
                        else:
                            print(
                                f"[bars_anchor_us] Derivation validated successfully for id={id_}"
                            )
        ...
        seconds = total_time % 60
        print(
            f"[bars_anchor_us] Full rebuild complete: {running_total:,} total rows [time: {minutes}m {seconds:.1f}s]"
        )
        return
