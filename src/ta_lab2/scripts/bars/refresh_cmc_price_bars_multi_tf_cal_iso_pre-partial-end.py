...
"""
Calendar-aligned price bars builder: public.cmc_price_bars_multi_tf_cal_iso
from public.cmc_price_histories7 (daily).

UPDATED (Incremental + dim_timeframe-driven):

1) Timeframes are loaded from public.dim_timeframe (no TF_LIST hardcoding):
   ...
   - tf is the label (e.g. '1W_ISO', '3M_CAL')

   Calendar scheme rules for this ISO-cal builder:

   - Weeks (base_unit='W'): calendar_scheme = 'ISO' (ISO weeks Mon–Sun)
   - Months/Years (base_unit in 'M','Y'): calendar_scheme = 'CAL' (month-end/year-end)

   (This matches your current dim_timeframe query output.)
...
2) Incremental refresh:
   ...
   - Backfill-aware: If earlier daily data arrives (daily_min decreases vs what
     ...
     because first-full-period anchoring can shift bar_seq.
...
Semantics preserved:
- Bars are calendar aligned:
    * Weeks: ISO weeks (Mon–Sun), closes on Sunday.
    * Months: month-aligned, closes on month-end.
    * Years: year-aligned, closes on Dec 31.
...
Quality-of-life improvements vs older version:
...
def load_cal_specs_from_dim_timeframe(db_url: str) -> list[CalSpec]:
    """
    Load ISO-week + CAL-month/year, full-period-only timeframes from dim_timeframe.
    ...
    Expected dim_timeframe conventions:
      ...
      - tf is label to emit (e.g. '1W_ISO', '3M_CAL', '1Y_CAL')

    Scheme rules:
      - Weeks (W): calendar_scheme='ISO'
      - Months/Years (M,Y): calendar_scheme='CAL'
    """
    sql = text("""
    ...
    for r in rows:
        ...
        n = int(r["tf_qty"])
        specs.append(CalSpec(n=n, unit=unit, tf=tf))

    if not specs:
        raise RuntimeError(
            "No ISO/CAL full-period timeframes found in dim_timeframe "
            "(alignment_type='calendar', allow_partial_* = false, base_unit in W/M/Y, "
            "scheme W->ISO and (M,Y)->CAL)."
        )
    return specs


# =============================================================================
# Calendar math helpers (local date logic)
# =============================================================================
...
def _bar_end_for_start(bar_start: date, n: int, unit: str) -> date:
    if unit == "W":
        return bar_start + timedelta(days=7 * n - 1)  # Sunday for 1W
    if unit == "M":
...
def _build_full_history_for_id_spec(df_id: pd.DataFrame, *, spec: CalSpec, tz: str) -> pd.DataFrame:
    ...
    while True:
        ...
        for _ in range(exp_n):
            ...
            if j is None:
                raise ValueError(
                    f"[bars_cal_iso] Missing daily rows for id={int(df_id['id'].iloc[0])}, tf={spec.tf}, "
    ...
    *,
    ...
    while True:
        ...
        for _ in range(exp_n):
            ...
            if j is None:
                raise ValueError(
                    f"[bars_cal_iso] Missing daily rows for id={int(df['id'].iloc[0])}, tf={spec.tf}, "
                    f"bar_start={bar_start}, bar_end={bar_end} (expected {exp_n} days)."
                )
            idxs.append(j)
            cur = cur + timedelta(days=1)
...
def refresh_incremental(
    *,
    ...
    for id_ in ids:
        ...
        for spec in specs:
            st = state_map.get((int(id_), spec.tf))
...
def main(argv: Sequence[str] | None = None) -> None:
    ap = argparse.ArgumentParser(
        description="Build ISO-week + CAL-month/year calendar-aligned price bars into public.cmc_price_bars_multi_tf_cal_iso (incremental)."
    )
