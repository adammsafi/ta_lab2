...
"""
# ======================================================================================
# refresh_cmc_price_bars_multi_tf_cal_anchor_iso.py (UPDATED)
#
    ...
    try:
        ...
        for spec in specs:
            try:
                ...
                # 2) Have prior state
                daily_min = (
                    pd.to_datetime(st["daily_min"], utc=True)
                    if pd.notna(st.get("daily_min"))
                    else pd.to_datetime(daily_min_ts, utc=True)
                )
                daily_max = (
                    pd.to_datetime(st["daily_max"], utc=True)
                    if pd.notna(st.get("daily_max"))
                    else pd.to_datetime(daily_max_ts, utc=True)
                )
                last_bar_seq = (
                    int(st["last_bar_seq"])
                    if pd.notna(st.get("last_bar_seq"))
                    else None
                )
                last_time_close = (
                    pd.to_datetime(st["last_time_close"], utc=True)
                    if pd.notna(st.get("last_time_close"))
                    else None
                )
                ...
                df_head = load_daily_prices_for_id(
                    db_url=db_url, daily_table=daily_table, id_=int(id_)
                )
                if df_head.empty:
                    stats["noops"] += 1
                ...
                first_window_start = _anchor_start_for_first_day(
                    first_day, spec.n, spec.unit
                )
                ...
                slice_start_day = max(
                    daily_min_day, start_day - timedelta(days=lookback)
                )
                ts_start_local = pd.Timestamp(
                    datetime.combine(slice_start_day, datetime.min.time()), tz=tz
                )
                ...
                last_row = load_last_snapshot_row(
                    db_url, bars_table, id_=int(id_), tf=spec.tf
                )
                ...
                stats["errors"] += 1
                print(f"[bars_anchor_iso] ERROR id={id_} tf={spec.tf}: {e}")

    except Exception as e:
        stats["errors"] += 1
        print(f"[bars_anchor_iso] ERROR id={id_}: {e}")
...
def _coerce_to_date(d) -> date:
    """
    Coerce numpy/pandas datetime-like values to python datetime.date.

    Needed because Polars -> numpy can yield numpy.datetime64[D],
    which breaks python date arithmetic.
    """
    if d is None:
        return d
    ...
    # Last resort: try pandas Timestamp
    return pd.Timestamp(d).date()


def _week_num_since_ref_iso(ref_monday_iso: date, d) -> int:
    """Number of full ISO weeks since REF_MONDAY_ISO (Monday-based)."""
    ref_monday_iso = _coerce_to_date(ref_monday_iso)
    d = _coerce_to_date(d)
    delta = (d - ref_monday_iso).days
    return delta // 7


def _anchor_window_for_day_iso_week(
    ref_monday_iso: date, n: int, d: date
) -> tuple[date, date]:
    """Return (window_start, window_end) for ISO N-week anchored window containing d (Mon..Sun)."""
    ref_week = _week_num_since_ref_iso(ref_monday_iso, d)
    group = ref_week // n
    first_week_in_group = group * n
    window_start = ref_monday_iso + timedelta(weeks=first_week_in_group)  # Monday
    window_end = window_start + timedelta(weeks=n) - timedelta(days=1)  # Sunday
...
def _anchor_window_for_day(d: date, n: int, unit: str) -> tuple[date, date]:
    ...
    if unit == "W":
        return _anchor_window_for_day_iso_week(REF_MONDAY_ISO, n, d)
    if unit == "M":
...
def _build_snapshots_full_history_for_id_spec_polars(
    ...
    *,
    ...
    if spec.unit == "W":
        # ISO weeks: Monday start, n-week windows
        ref = pd.Timestamp(REF_MONDAY_ISO)
        days_since_ref = (dates_dt - ref).dt.days
        week_nums = days_since_ref // 7
        groups = week_nums // spec.n
        window_start_days = groups * spec.n * 7
        df["window_start"] = (
            ref + pd.to_timedelta(window_start_days, unit="D")
        ).dt.date
        df["window_end"] = (
            ref + pd.to_timedelta(window_start_days + spec.n * 7 - 1, unit="D")
        ).dt.date
        df["tf_days"] = spec.n * 7
    elif spec.unit == "M":
        ...
        df["window_end"] = [
            date(int(fy) + spec.n, 1, 1) - timedelta(days=1) for fy in first_years
        ]
        df["tf_days"] = [
            (date(int(fy) + spec.n, 1, 1) - date(int(fy), 1, 1)).days
            for fy in first_years
        ]
    else:
    ...
    df["timehigh"] = pd.to_datetime(
        df["timehigh"], utc=True, errors="coerce"
    ).dt.tz_localize(None)
    df["timelow"] = pd.to_datetime(
        df["timelow"], utc=True, errors="coerce"
    ).dt.tz_localize(None)

    pl_df = pl.from_pandas(df).sort("ts")

    # Add tf early (daily source doesn't include it; builder is per-spec)
    pl_df = pl_df.with_columns(pl.lit(spec.tf).alias("tf"))
    ...
    pl_df = pl_df.with_columns(
        [
            (pl.col("bar_anchor_offset").rank(method="dense"))
            .cast(pl.Int64)
            .alias("bar_seq")
        ]
    )
    ...
    pl_df = pl_df.with_columns(
        [
            ...
            .first()
            .over("bar_seq")
            .alias("_first_local_date_in_bar"),
        ]
    )
    ...
    pl_df = pl_df.with_columns(
        [
            (
                (pl.col("_first_local_dt") - pl.col("bar_start_eff_dt"))
                .dt.total_days()
                .clip(lower_bound=0)
            )
            .cast(pl.Int64)
            .alias("count_missing_days_start"),
        ]
    ...
    pl_df = pl_df.with_columns(
        [
            ...
            pl.lit(0)
            .cast(pl.Int64)
            .alias("count_missing_days_end"),  # snapshot_day always exists
            pl.max_horizontal(
    ...
    pl_df = pl_df.with_columns(
        [
            ((pl.col("_local_dt") - pl.col("_bse_dt")).dt.total_days() + 1)
            .cast(pl.Int64)
            .alias("exp_to_date"),
        ]
    ...
    pl_df = pl_df.with_columns(
        [
            (pl.col("tf_days").cast(pl.Int64) - pl.col("exp_to_date"))
            .cast(pl.Int64)
    ...
    # Re-add UTC timezone to timestamp columns
    timestamp_cols = ["time_open", "time_close", "time_high", "time_low"]
    from ta_lab2.scripts.bars.polars_bar_operations import (
    ...
    *,
    ...
    while curr_day <= end_day:
        ...
        if can_carry:
            ...
            # Conditional high recomputation
            if df_window["high"].max() > last_snapshot_row["high"]:
                time_high, _ = compute_time_high_low(
                    df_window,
                    ts_col="ts",
                    high_col="high",
                    low_col="low",
            ...
            # Conditional low recomputation
            if df_window["low"].min() < last_snapshot_row["low"]:
                _, time_low = compute_time_high_low(
                    df_window,
                    ts_col="ts",
                    high_col="high",
                    low_col="low",
            ...
            time_high, time_low = compute_time_high_low(
                df_window,
                ts_col="ts",
                high_col="high",
                low_col="low",
        ...
        expected_dates = {
            bar_start_eff + timedelta(days=i)
        ...
        (
            count_missing,
            count_missing_start,
            count_missing_interior,
            count_missing_end,
            missing_where,
        ) = _compute_missing_days_breakdown(
            expected_dates, available_dates, bar_start_eff, snapshot_day
        )
        ...
        results.append(
            {
                ...
                "time_high": pd.to_datetime(time_high, utc=True)
                if not pd.isna(time_high)
                else None,
                "time_low": pd.to_datetime(time_low, utc=True)
                if not pd.isna(time_low)
                else None,
                "open": float(open_val),
                "high": float(high_val),
                "low": float(low_val),
...
def refresh_incremental(
    *,
    ...
    if not specs:
        print("[bars_anchor_iso] No specs found in dim_timeframe. Nothing to do.")
        return

    total_combinations = len(ids) * len(specs)
    print(
        f"[bars_anchor_iso] Incremental: {len(ids)} IDs × {len(specs)} TFs = {total_combinations:,} combinations (tz={tz})"
    )

    # Load daily min/max per id (for backfill detection / range)
    minmax = load_daily_min_max(db_url=db_url, daily_table=daily_table, ids=ids)
    if minmax.empty:
        print("[bars_anchor_iso] No daily rows found for requested ids.")
        return

    # Preload state for requested ids (batch)
    st_df = load_state(db_url, state_table, ids, with_tz=True)

    state_map: dict[tuple[int, str], dict] = {}
    if not st_df.empty:
        for r in st_df.to_dict("records"):
            state_map[(int(r["id"]), str(r["tf"]))] = r

    # Build per-id state lookup map once
    def _state_map_for_id(id_: int) -> dict[tuple[int, str], dict]:
        out = {}
        for spec in specs:
            k = (int(id_), spec.tf)
            if k in state_map:
    ...
    args_list = []
    for id_ in ids:
        row = minmax[minmax["id"] == int(id_)]
        if row.empty:
            continue
        daily_min_ts = row.iloc[0]["daily_min"]
        daily_max_ts = row.iloc[0]["daily_max"]
        args_list.append(
            (
                int(id_),
                db_url,
                ...
                daily_min_ts,
                daily_max_ts,
                _state_map_for_id(int(id_)),
                fail_on_internal_gaps,
            )
        )

    if not args_list:
        print("[bars_anchor_iso] Nothing to do (no ids with data).")
        return

    print(
        f"[bars_anchor_iso] Processing ids={len(args_list)} with num_processes={num_processes}"
    )
    ...
    progress = ProgressTracker(
        total=len(args_list), log_interval=5, prefix="[bars_anchor_iso]"
    )
    ...
    print(
        f"[bars_anchor_iso] Incremental complete: upserted={agg_stats['upserted']:,} "
        f"rebuilds={agg_stats['rebuilds']} appends={agg_stats['appends']} noops={agg_stats['noops']} "
        f"errors={agg_stats['errors']} [time: {minutes}m {seconds:.1f}s]"
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.from_1d:
        ...
        print(
            f"[bars_anchor_iso] Deriving anchored calendar {alignment} bars from cmc_price_bars_1d (partial periods allowed)"
        )
        if args.validate_derivation:
            print(
                "[bars_anchor_iso] Validation mode: will compare derived bars to direct computation"
            )
    ...
    if args.full_rebuild:
        ...
        print(
            f"[bars_anchor_iso] Full rebuild: {len(ids)} IDs × {len(specs)} TFs = {total_combinations:,} combinations"
        )

        # Ensure state table exists (tz is part of state schema for *_cal_anchor_* tables)
        ensure_state_table(db_url, args.state_table, with_tz=True)

        # Get alignment mode for derivation (anchor_mode=True for anchor builders)
        alignment, anchor_mode = BUILDER_ALIGNMENT_MAP["cal_anchor_iso"]
        eng = get_engine(db_url)
        ...
        for id_ in ids:
            ...
            if args.from_1d:
                # Derive all bars from 1D source with anchor_mode=True
                ...
                if args.validate_derivation:
                    ...
                    if bars_direct_all:
                        ...
                        if not is_consistent:
                            print(
                                f"[bars_anchor_iso] Derivation discrepancies for id={id_}: {discrepancies}"
                            )
                        else:
                            print(
                                f"[bars_anchor_iso] Derivation validated successfully for id={id_}"
                            )
                ...
                if not bars_all.is_empty():
                    ...
                    for spec in specs:
                        ...
                        if not bars_tf.empty:
                            ...
                            print(
                                f"[bars_anchor_iso] ID={id_}, TF={spec.tf}, period={period_start} to {period_end}: "
                                f"upserted {num_rows:,} rows (DERIVED ANCHOR, {running_total:,} total, {pct:.1f}%) [elapsed: {elapsed:.1f}s]"
                            )
                continue
            ...
            for spec in specs:
                ...
                if not df_full.empty:
                    state_row = {
                        "id": int(id_),
                        "tf": spec.tf,
                        "tz": args.tz,  # state table tz is NOT NULL
        ...
        seconds = total_time % 60
        print(
            f"[bars_anchor_iso] Full rebuild complete: {running_total:,} total rows [time: {minutes}m {seconds:.1f}s]"
        )
        return
    ...
    refresh_incremental(
        ...
        fail_on_internal_gaps=bool(args.fail_on_internal_gaps),
    )
