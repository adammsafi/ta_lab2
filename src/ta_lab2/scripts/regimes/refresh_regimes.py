"""
refresh_regimes.py - Core regime refresh script.
...
all 4 regime tables (regimes, regime_flips, regime_stats,
regime_comovement).
...
Exports:
    compute_regimes_for_id: Per-asset computation returning regimes-shaped DataFrame
    write_regimes_to_db:    Scoped DELETE + INSERT write to regimes
    main:                   CLI entrypoint
"""
...
def _load_macro_regime_with_staleness_check(
    ...
    """
    Load the latest macro regime key from macro_regimes and validate freshness.
    ...
    - macro_regimes does not exist (Phase 67 not yet applied)
    ...
    query = text(
        "SELECT regime_key, date FROM public.macro_regimes "
    ...
    try:
        ...
        _try_telegram_alert(
            "regime_refresh",
            f"L4 macro regime disabled: macro_regimes not available ({exc}). "
    ...
    if row is None:
        logger.warning(
            "_load_macro_regime_with_staleness_check: macro_regimes is empty -- "
            "L4 disabled (profile=%s)",
            profile,
        )
        _try_telegram_alert(
            "regime_refresh",
            f"L4 macro regime disabled: macro_regimes is empty (profile={profile}). "
...
def compute_regimes_for_id(
    ...
    Returns:
        DataFrame with columns matching regimes schema, one row per
...
def write_regimes_to_db(
    ...
    """
    Write regime rows to regimes using scoped DELETE + INSERT.
    ...
    Args:
        ...
        df:     DataFrame with columns matching regimes schema.
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    Example usage:
        ...
        python -m ta_lab2.scripts.regimes.refresh_regimes --all --cal-scheme iso
        python -m ta_lab2.scripts.regimes.refresh_regimes --ids 1 --no-hysteresis
        python -m ta_lab2.scripts.regimes.refresh_regimes --ids 1 --min-hold-bars 5
    """
    parser = argparse.ArgumentParser(
        description="Refresh regimes: compute regime labels for all assets.",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable rolling stats augmentation from asset_stats. "
    ...
    for asset_id in asset_ids:
        try:
            ...
            # Load rolling stats from asset_stats and merge into daily_df.
            ...
            if not args.dry_run:
                ...
                # Write regime_comovement (compute + write)
