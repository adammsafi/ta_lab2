...
def _norm_cols(df):
    df = df.copy(); df.columns = [re.sub(r"\s+"," ",c.strip().lower()) for c in df.columns]; return df
...
def load_df(path):
    ...
    d[ts] = pd.to_datetime(d[ts], errors="coerce"); d = d.dropna(subset=[ts]).set_index(ts).sort_index()
    try: d.index = d.index.tz_localize(None)
    except: pass
    if px != "close": d = d.rename(columns={px:"close"})
...
def main():
    ...
    ap.add_argument("--strategy", default="ema_trend", help="Strategy key from registry")
    ap.add_argument("--start", default="2017-01-01")
    ap.add_argument("--end",   default="2024-12-30")
    ap.add_argument("--out",   default=r"C:\Users\asafi\Downloads\ta_lab2\research\outputs\opt_generic.csv")
    ...
    outdir = Path(args.out).parent; os.makedirs(outdir, exist_ok=True)
    ...
    print(res.head(12)[["split","p_fast_ema","p_slow_ema","trades","total_return","cagr","mdd","mar","sharpe"]]
          if "p_fast_ema" in res.columns else
          res.head(12)[["split","trades","total_return","cagr","mdd","mar","sharpe"]])
