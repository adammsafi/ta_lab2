...
OUT     = r"C:\Users\asafi\Downloads\ta_lab2\research\outputs\opt_cf_ema_sensitivity.csv"
...
def build_grid(f: int, s: int, f_pad=10, s_pad=20, delta=5):
    ...
    return [
        ...
        for fi in fasts for si in slows if si >= fi + delta
    ]
...
def main():
    ...
    # Precompute EMA spans in the sensitivity neighborhood (FIX: use set union correctly)
    spans = set(range(max(5, f - 10), f + 11)).union(set(range(max(10, s - 20), s + 21)))
    for span in spans:
        ensure_ema(df, span)
