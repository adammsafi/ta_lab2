...
Asset selectors are placed inline next to their sections (outside fragments).
"""
...
# ---------------------------------------------------------------------------
# Load perp list for dropdowns (needed before selectors)
# ---------------------------------------------------------------------------
...
def _top_perps_section(_engine, days_back: int) -> None:
    """Top perps by volume: metric cards + table."""
    ...
    try:
        ...
        for i, (col, row) in enumerate(zip(cols, top3.itertuples())):
            ...
            funding_str = (
                f"{float(row.funding) * 100:.4f}%" if row.funding is not None else "N/A"
            )
            col.metric(
        ...
        if "open_interest" in display_df.columns and "mark_px" in display_df.columns:
...
_top_perps_section(engine, days_back=days_back)

st.divider()

# ===========================================================================
# Section 2: Funding Rate Analysis -- selectors inline
# ===========================================================================

st.subheader("Funding Rate Analysis")

_fund_col1, _fund_col2 = st.columns(2)
with _fund_col1:
...
def _funding_section(
    ...
    with tab_multi:
        if not multi_asset_ids:
            st.info("Select up to 5 perps for comparison.")
        else:
            try:
...
# ===========================================================================
# Section 4: Candle Chart + OI -- selector inline
# ===========================================================================
