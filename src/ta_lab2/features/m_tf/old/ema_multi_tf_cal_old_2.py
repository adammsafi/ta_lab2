...
def build_multi_timeframe_ema_cal_frame(
    ...
    *,
    ...
    # roll_bar: FALSE where we have a true bar EMA (ema_bar not null),
    #           TRUE everywhere else.
    is_bar = result_df["ema_bar"].notna()
    result_df["roll_bar"] = ~is_bar

    # Per-day bar-state diffs: d1_roll_bar, d2_roll_bar
    # We treat ema_bar as a bar-state that is held constant (ffilled)
    # between closes, then jumps at each new bar close.
    result_df["ema_bar_ffill"] = g_full["ema_bar"].ffill()
    result_df["d1_roll_bar"] = g_full["ema_bar_ffill"].diff()
    result_df["d2_roll_bar"] = g_full["d1_roll_bar"].diff()

    # Closing-only bar diffs: d1_bar, d2_bar (only on TF closes)
    result_df["d1_bar"] = np.nan
    result_df["d2_bar"] = np.nan

    mask_bar_close = ~result_df["roll_bar"]
    ...
    # We don't need ema_bar_ffill in the final output
    result_df = result_df.drop(columns=["ema_bar_ffill"])
...
def write_multi_timeframe_ema_cal_to_db(
    ...
    # (via load_cmc_ohlcv_daily) hits the *same* database.
    try:
        db_url = str(engine.url)
    except Exception:
        db_url = None  # fallback to io._get_marketdata_engine() defaults, if any
