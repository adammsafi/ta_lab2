...
def expand_datetime_features_inplace(
    ...
    *,
    ...
    to_utc : bool
        ...
        timestamps to UTC. Astronomy and season boundaries are computed in UTC.
    add_moon : bool
        If True and `astronomy` is available, adds moon phase features.

    Notes
    -----
    - This is NaT-safe: rows with unparseable timestamps will yield NA features.
    - Restores legacy fields expected by tests:
    ...
    # --- Base parts ---
    us_dow = ((dt.dt.dayofweek + 1) % 7 + 1).astype("Int64")  # Sun=1..Sat=7 (US style)
    ...
    # Month boundaries (warnings about tz drop are harmless for derived features)
    month_start = dt.dt.to_period("M").dt.start_time
    month_end = dt.dt.to_period("M").dt.end_time

    # Business-day flag (fix for pandas dtype: cast to date64 before is_busday)
    # Use normalize() then convert to numpy datetime64[D].
    ...
    # Friendly log
    # print(f"Expanded datetime features for: '{base_timestamp_col}'")
