...
def convert_excel_to_markdown(
    ...
    logger.info(f"Converting {excel_path.name} to Markdown{' (dry run)' if dry_run else ''}")
    ...
    try:
        ...
        for sheet_name in sheets_to_convert:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"to_markdown failed for '{sheet_name}', using fallback: {e}")
                    # Add HTML comment about limitations
                    markdown_lines.append("<!-- Complex formatting may not be preserved -->")
                    markdown_lines.append("")
                    # Simple table fallback
                    markdown_lines.append("| " + " | ".join(str(col) for col in df.columns) + " |")
                    markdown_lines.append("| " + " | ".join("---" for _ in df.columns) + " |")
                ...
                logger.debug(f"Converted sheet '{sheet_name}': {len(df)} rows x {len(df.columns)} columns")
        ...
        if not dry_run:
            ...
            logger.info(f"[DRY RUN] Would write: {output_path} ({sheets_converted} sheets)")
...
def batch_convert_excel(
    ...
    if not input_dir.exists():
        ...
        return ConversionResult(total=0, converted=0, skipped=0, errors=1, error_paths=[str(input_dir)])

    logger.info(f"Batch converting Excel files from {input_dir}{' (dry run)' if dry_run else ''}")
