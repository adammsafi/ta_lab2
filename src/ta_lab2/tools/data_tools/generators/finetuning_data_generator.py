...
def generate_question_for_memory(memory: Dict[str, Any], client: OpenAI, model: str) -> str | None:
    """
...
def main() -> int:
    ...
    memories_to_process = memories[:args.max_memories] if args.max_memories > 0 else memories

    log.info(f"Generating questions for {len(memories_to_process)} memories using {args.question_gen_model}...")
    ...
    with output_path.open("w", encoding="utf-8") as f:
        for i, mem in enumerate(memories_to_process):
            log.info(f"Processing memory {i+1}/{len(memories_to_process)}: '{mem.get('title', 'Untitled')}'")
            
            question = generate_question_for_memory(mem, client, args.question_gen_model)
            
            if question:
                training_sample = {
                    "messages": [
                        {"role": "system", "content": "You are an expert developer on the ta_lab2 project, providing answers based on its established knowledge base."},
                ...
                log.warning(f"Skipping memory {i+1} due to question generation failure.")
            
            if (i + 1) % 10 == 0: # Log progress every 10 memories
                elapsed = time.time() - start_time
                log.info(f"Progress: {i+1}/{len(memories_to_process)} processed. Generated {generated_count} samples. Elapsed: {elapsed:.1f}s")


    log.info(f"Successfully generated {generated_count} fine-tuning samples to: {output_path}")
    log.info(f"Total time for question generation: {time.time() - start_time:.1f} seconds.")
    log.info("Remember to manually review and refine the generated training_data.jsonl file for best results!")
