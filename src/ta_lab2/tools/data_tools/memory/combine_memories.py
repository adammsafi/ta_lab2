                ...
                with open(file_path, "r", encoding="utf-8") as infile:
                    for line in infile:
                        try:
                            ...
                            print(
                                f"Warning: Skipping invalid JSON line in {file_path}: {line.strip()}"
                            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Combine multiple .jsonl files into a single file, ensuring unique memories based on content."
    )
    parser.add_argument("output_file", help="The path to the output .jsonl file.")
    parser.add_argument(
        "input_files",
        nargs="+",
        help="The paths to the input .jsonl files to be combined.",
    )
    ...
    print(
        f"Successfully combined {len(args.input_files)} files into '{args.output_file}'."
    )
