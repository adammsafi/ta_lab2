...
def _find_default_csv() -> str | None:
    for p in [
        "data/btc.csv", "data/BTC.csv", "data/btcusd.csv", "data/btc_usd.csv", "btc.csv"
    ]:
        if (HERE / p).exists():
            return str(HERE / p)
...
def _ensure_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ts = "timestamp" if "timestamp" in d.columns else next(
        (c for c in d.columns if "time" in c or "date" in c), d.columns[0]
    )
    if ts != "timestamp":
...
def monthly_returns(DF: pd.DataFrame):
    ...
    harm_rel = grp.apply(lambda g: len(g) / np.sum(1.0 / daily_rel.loc[g.index].clip(lower=1e-9)))
    harm = (harm_rel - 1.0).rename("harm_ret")
    out = pd.concat([arith, geo, harm], axis=1)

    # plot last 18 months (arith)
    fig, ax = plt.subplots(figsize=(14, 5))
    out["arith_ret"].tail(18).plot(kind="bar", ax=ax, title="Monthly Arithmetic Returns (last 18)")
...
def rolling_compare_returns(DF: pd.DataFrame, window=90):
    ...
    geo = (pct.add(1).rolling(window).apply(np.prod, raw=True)**(1.0/window) - 1.0).rename(f"geo_{window}")
    rel = pct.add(1).clip(lower=1e-9)
    harm = (window / rel.rolling(window).apply(lambda a: np.sum(1.0/np.maximum(a,1e-9)), raw=True) - 1.0).rename(f"harm_{window}")
    dm = pd.concat([arith, geo, harm], axis=1)

    fig, ax = plt.subplots(figsize=(14, 6))
    dm.tail(1000).plot(ax=ax, lw=1.1, title=f"Rolling daily averages ({window}) — arithmetic vs geometric vs harmonic")
...
def comovement_daily_vs_weekly(DF: pd.DataFrame):
    ...
    d_ema = {
        ...
        180: daily.ewm(alpha=2/(180+1), adjust=False).mean().reindex(z.index).ffill(),
        270: daily.ewm(alpha=2/(270+1), adjust=False).mean().reindex(z.index).ffill(),
    }
    # weekly EMAs (25/35/45) on weekly resample
    weekly = z["close"].resample("W").last().dropna()
    w_ema = {
        25: weekly.ewm(alpha=2/(25+1), adjust=False).mean().reindex(z.index).ffill(),
        35: weekly.ewm(alpha=2/(35+1), adjust=False).mean().reindex(z.index).ffill(),
        45: weekly.ewm(alpha=2/(45+1), adjust=False).mean().reindex(z.index).ffill(),
    }
    dfc = pd.DataFrame({
        "d_ema_90": d_ema[90], "d_ema_180": d_ema[180], "d_ema_270": d_ema[270],
        "w_ema_25": w_ema[25], "w_ema_35": w_ema[35], "w_ema_45": w_ema[45],
        "close": z["close"]
    }).dropna()
    ...
    dfc["daily_all_up"]  = (dfc["d_ema_90_slope"]>0) & (dfc["d_ema_180_slope"]>0) & (dfc["d_ema_270_slope"]>0)
    dfc["weekly_all_up"] = (dfc["w_ema_25_slope"]>0) & (dfc["w_ema_35_slope"]>0) & (dfc["w_ema_45_slope"]>0)
    ...
    dd = dfc.tail(1000)
    fig, ax = plt.subplots(figsize=(14,6))
    dd["close"].plot(ax=ax, lw=1.2, title="Close with Daily vs Weekly EMA regime agreement (last 1000)")
    (dd["daily_all_up"].astype(int)*dd["close"].rolling(10).mean()).plot(ax=ax, alpha=0.35)
    (dd["weekly_all_up"].astype(int)*dd["close"].rolling(10).mean()).plot(ax=ax, alpha=0.35)
...
def _boot():
    ...
    if not csv:
        raise FileNotFoundError("No input CSV found. Drop one at data/btc.csv or adjust _find_default_csv().")
    raw = pd.read_csv(csv)
    df = _ensure_features(raw)
    globals()["DF"] = df  # expose to Variable Explorer
    print(f"DF ready, shape={df.shape}. Try monthly_returns(DF), rolling_compare_returns(DF, 90), comovement_daily_vs_weekly(DF), flip_segments_quick(DF), full_pipeline_summary(csv).")
    return csv, df
...
def comovement_menu(DF):
    d_per = [int(x) for x in input("Daily EMA periods (comma): ").split(",")]  # e.g., 90,180,270
    w_per = [int(x) for x in input("Weekly EMA periods (comma): ").split(",")] # e.g., 25,35,45
    ...
    res = disk_cache("comovement_v2", _compute, d_periods=sorted(d_per), w_periods=sorted(w_per))
    globals()["COMOVEMENT"] = res["table"]
    print(f"Agreement: {res['agree_pct']:.2f}%")
    res["table"].tail(1000)["close"].plot(figsize=(12,5)); plt.show()
...
FREQS = [
    "2D","3D","4D","5D","10D","25D","45D",   # calendar-day bins
    "W", "2W", "3W",                         # weeks (Sunday-end) 
    "W-FRI",                                 # market-friendly week
    "M","2M","3M","6M","A"                   # month & annual (‘A’ ~ 1Yr)
]
